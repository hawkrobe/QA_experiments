...
Fit questioner parameters 

```{r}
d_q <- tbl_df(d_short_q) %>%  # start with empirical data
...
# Plot param space
g_qParams <- (ggplot(df_correlation, aes(x = ansR, y =KLR))
       + geom_tile(aes(fill = correlation))
       + geom_point(x = max_val$ansR, y = max_val$KLR, 
                    color = 'white', size = 5)
       + scale_fill_gradient(low = "white", high = "black"))
g_qParams

# Plot best model
max_val = df_correlation[which.max(df_correlation$correlation),]
best_pred = subset(d_q, ansR == max_val$ansR & KLR == max_val$KLR)
g_bestQ <- (ggplot(best_pred, aes(x = model_prob, y = emp_prob))
            + geom_point() 
...
```{r}
model_pred <- read.csv("analysis/model_prediction/pragAnswererRationalityFitting.csv", 
                     sep =',') %>% # join with model predictions
             filter(response %in% c("poodle", "dalmatian", 
                                    "siamese cat", "goldfish")) 
d_a <- tbl_df(d_short_a) %>%  # start with empirical data
...
jpeg(filename="../writing/2015/cogsci/AnswererRationalityTuning.jpeg")
...
# Plot param space
g_aParams <- (ggplot(a_correlation, aes(x = ansR, y =KLR))
       + geom_tile(aes(fill = correlation))
       + geom_point(x = max_val$ansR, y = max_val$KLR, 
                    color = 'white', size = 5)
       + scale_fill_gradient(low = "white", high = "black"))
...
# Plot best model
max_val = a_correlation[which.max(a_correlation$correlation),]
best_pred = subset(d_a, ansR == max_val$ansR & KLR == max_val$KLR 
                        & qudR == max_val$qudR & pragR == max_val$pragR)
g_bestA <- (ggplot(best_pred, aes(x = model_prob, y = emp_prob))
            + geom_point() 
            + annotate("text", x = .5, y = .75, 
                       label = paste("R^2 = ", round(max_val$correlation,2)))
            + geom_smooth(method = "lm"))
g_bestA
```
