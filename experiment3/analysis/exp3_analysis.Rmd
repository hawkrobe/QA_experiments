...
```{r}
d_short = read.csv("versions/experiment3_short/data/q_and_a3_short-trials.tsv", 
                   sep = '\t')
...
d_short_q$response = factor(d_short_q$response)
new_labels = sapply(X = d_short_q$qud, 
                    FUN = function(v) {return(paste("qud:", v))})
d_short_q$facet_label = new_labels
...
new_labels = sapply(X = d_short_a$utterance, 
                    FUN = function(v) {return(paste("utterance:", v))})
d_short_a$facet_label = new_labels
g4<-(ggplot(d_short_a, aes(x=item_response)) 
     + geom_histogram(color="black", fill='black')
     + facet_wrap(~facet_label))
g4
```
...
```{r}
pred_ll = read.csv("analysis/litListenerModelPredictions.csv", sep = '\t')
new_labels = sapply(X = pred_ll$utterance, FUN = function(v) {return(paste("utterance:", v))})
pred_ll$facet_label = new_labels
g4<-ggplot(pred_ll, aes(x=response, y=prob)) + geom_bar(stat='identity', color='black', fill='black')
g4 + facet_wrap(~facet_label)
```
...
```{r}
pred_prag = read.csv("analysis/pragAnswererModelPredictions.csv", sep = '\t')
new_labels = sapply(X = pred_prag$utterance, FUN = function(v) {return(paste("utterance:", v))})
pred_prag$facet_label = new_labels
