...
```{r}
qparams<-read.csv("guessingGame/Bayesian/data/BayesianQuestionerAnalysisParams_flipBothOutside10000.csv", sep = ",", row.names = NULL)
samples = 1000
str(qparams)
...
md_hi = round(HPDhi(subset(qparams.samples, parameter == "beta")$value), 3)
md_low = round(HPDlo(subset(qparams.samples, parameter == "beta")$value), 3)
cat("beta = ", md) 
cat("95% HPD interval = [", md_low, ",", md_hi, "]")

md = estimate_mode(subset(qparams.samples, parameter == "expVsPrag")$value)
md_hi = round(HPDhi(subset(qparams.samples, parameter == "expVsPrag")$value), 3)
md_low = round(HPDlo(subset(qparams.samples, parameter == "expVsPrag")$value), 3)
cat("expVsPrag = ", md) 
cat("95% HPD interval = [", md_low, ",", md_hi, "]")
...
ggplot(qparams.samples, aes(x=value))+
    geom_histogram(aes(y=..density..),data =subset(qparams.samples, parameter == "alpha"), binwidth = .25, colour="black", fill="white")+
...
```{r}
aparams<-read.csv("guessingGame/Bayesian/data/BayesianAnswererAnalysisParams_betaFlipOutside.csv", sep = ",", row.names = NULL)
samples = 1000
str(aparams)
...
```{r}
qpredictive<-read.csv("guessingGame/Bayesian/data/BayesianQuestionerAnalysisPredictives_flipBothOutside10000.csv", sep = ",", row.names = NULL)
