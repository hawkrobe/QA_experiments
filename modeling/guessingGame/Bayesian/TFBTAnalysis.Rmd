...
```{r}
qparams<-read.csv("guessingGame/Bayesian/data/BayesianQuestionerAnalysisParams_diffBeta.csv", sep = ",", row.names = NULL)
samples = 1000
str(qparams)
...
ggplot(aparams.samples, aes(x=value))+
    geom_histogram(aes(y=..density..), data =subset(aparams.samples, parameter == "alpha" & value <= 10), binwidth = .25, colour="black", fill="white")+
    geom_histogram(aes(y=..density..), data=subset(aparams.samples, parameter == "beta"), binwidth = .04, colour="black", fill="white")+
    geom_density(data =subset(aparams.samples, parameter == "alpha" & value < 10), adjust = 2,
                 alpha=.2, fill="#FF6666")+
...
cor(apredictive.pp$MAP, apredictive.pp$empProb)
...
answer_plots = (ggplot(apredictive.pp, aes(x = MAP, y = empProb))
  ...
  + geom_text(aes(x=.75,y=.25,label="r=0.92"))
  + theme_bw())
answer_plots
dev.off()
```
