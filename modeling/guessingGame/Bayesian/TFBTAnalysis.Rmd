...
qparams_raw %>% filter(parameter == "expPragFlip") %>% group_by(value) %>% summarize(prob = sum(MCMCprob))
```
...
```{r}
filteredQparams = qparams_raw %>% filter(MCMCprob > 0.0001)
samples = 1/0.0001
...
```{r}
raw_aparams<-read.csv("guessingGame/Bayesian/data/BayesianAnswererAnalysis_loose_params.csv", sep = ",", row.names = NULL) 

# BDA result:
raw_aparams %>% filter(parameter == "expPragFlip") %>% group_by(value) %>% summarize(prob = sum(MCMCprob))
```

```{r}
filteredAparams<-read.csv("guessingGame/Bayesian/data/BayesianAnswererAnalysis_loose_params.csv", sep = ",", row.names = NULL) %>%
...
aparams.samples <- filteredAparams[rep(row.names(filteredAparams), filteredAparams$MCMCprob*samples), 1:2]
...
answererParamPosteriors = ggplot(aparams.toplot, aes(x=value))+
    ...
    geom_histogram(aes(y=..density..), 
                   ...
                   binwidth = .04, colour="black", fill="white") +
