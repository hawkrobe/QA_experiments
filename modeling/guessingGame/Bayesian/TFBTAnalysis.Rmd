...
```{r}
qparams<-read.csv("guessingGame/Bayesian/data/BayesianQuestionerAnalysisParams_final.csv", sep = ",", row.names = NULL)
samples = 10000
str(qparams)
qparams.samples <- qparams[rep(row.names(qparams), qparams$MCMCprob*samples), 1:2]

alphaSubset = subset(qparams.samples, parameter == "alpha")$value
md = estimate_mode(as.numeric(alphaSubset))
md_hi = round(HPDhi(as.numeric(alphaSubset)), 3)
md_low = round(HPDlo(as.numeric(alphaSubset)), 3)
cat("alpha = ", md) 
cat("95% HPD interval = [", md_low, ",", md_hi, "]")

betaSubset = subset(qparams.samples, parameter == "beta")$value
md = estimate_mode(as.numeric(betaSubset))
md_hi = round(HPDhi(as.numeric(betaSubset)), 3)
md_low = round(HPDlo(as.numeric(betaSubset)), 3)
cat("beta = ", md) 
cat("95% HPD interval = [", md_low, ",", md_hi, "]")

eVpSubset = subset(qparams.samples, parameter == "expVsPrag")$value
md = estimate_mode(as.numeric(eVpSubset))
md_hi = round(HPDhi(as.numeric(eVpSubset)), 3)
md_low = round(HPDlo(as.numeric(eVpSubset)), 3)
cat("expVsPrag = ", md) 
cat("95% HPD interval = [", md_low, ",", md_hi, "]")
...
ggplot(qparams.samples, aes(x=value)) +
    geom_histogram(data =subset(qparams.samples, parameter == "alpha"), 
                   binwidth = .3, colour="black", fill="white")+
    geom_histogram(data=subset(qparams.samples, parameter == "beta"), 
                   binwidth = .025, colour="black", fill="white")+
    geom_histogram(data=subset(qparams.samples, parameter == "expVsPrag"),
                   binwidth = .05, colour="black", fill="white")+
    geom_density(aes(y=.3*..count..), data =subset(qparams.samples, parameter == "alpha"), adjust = 3.5,
                 alpha=.2, fill="#FF6666")+
    geom_density(aes(y=.025*..count..),data=subset(qparams.samples, parameter == "beta"), adjust = 3.5,
                 alpha=.2, fill="#FF6666")+
    geom_density(aes(y=.05*..count..),data=subset(qparams.samples, parameter == "expVsPrag"),adjust = 2,
                 alpha=.2, fill="#FF6666")+
    ggtitle("Questioner Parameter Posteriors (10000 iterations)") +
