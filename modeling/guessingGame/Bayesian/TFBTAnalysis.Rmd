...
ggplot(qparams.samples, aes(x=value))+
    geom_histogram(aes(y=..density..),data =subset(qparams.samples, parameter == "alpha"), binwidth = .12, colour="black", fill="white")+
    geom_histogram(aes(y=..density..),data=subset(qparams.samples, parameter == "beta"), binwidth = .05, colour="black", fill="white")+
    geom_histogram(aes(y=..density..),data=subset(qparams.samples, parameter == "expVsPrag"),binwidth = .04, colour="black", fill="white")+
    geom_density(data =subset(qparams.samples, parameter == "alpha"), adjust = 3,
                 alpha=.2, fill="#FF6666")+
    geom_density(data=subset(qparams.samples, parameter == "beta"), adjust = 3,
                 alpha=.2, fill="#FF6666")+
    geom_density(data=subset(qparams.samples, parameter == "expVsPrag"),adjust = 0.4,
                 alpha=.2, fill="#FF6666")+
...
answer_plots = (ggplot(apredictive.pp, aes(x = MAP, y = empProb))
  ...
  + geom_errorbar(aes(ymax = upper_ci, ymin = lower_ci, alpha = .25)) 
  + geom_errorbarh(aes(xmax = credHigh, xmin = credLow, alpha = .25))
...
cor(qpredictive.pp$MAP, qpredictive.pp$empProb)
...
question_plots = (ggplot(qpredictive.pp, aes(x = MAP, y = empProb))
  ...
  + geom_smooth(method = "lm")
  + geom_text(aes(x=.75,y=.25,label="r=0.976"))
  + theme_bw())
question_plots
dev.off()
```