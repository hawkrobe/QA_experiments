...
First, it depends on $P_{A_i}(a | q, w^*)$, a model of likely answerer behavior which we will explore shortly. We index the questioner and answerer models by $i$ because we want to formulate different questioner models $Q_i$ which reason about different answerers $A_i$ (see Figure \ref{fig:models}).
...
The set of answers $\mathcal{A}$ includes exact prices $a~\in~\{\$1, \$2, \dots, \$10\}$ as well as ``yes'' and ``no'', with higher prior probability (lower cost) for ``yes'' and ``no'' than the price statements. More specifically, the answer prior is hierarchical, assigning equal probability to picking the category of ``yes/no'' answers and of ``price'' answers, then uniformly draws from the possible responses within each category. Thus, $$P(\textrm{``yes''}) = P(\textrm{``no''}) = 0.25$$ and $$P(\textrm{``\$1''}) = \cdots = P(\textrm{``\$10''}) = 0.05$$
...
\hline
``Some bourbon'' &   0.41 & 0.59 &  0.37 & 0.62 \\
\hline
...
Our pragmatic answerer model's output is shown in Table \ref{table:clark79exp4}, compared with empirical results from \citeA{Clark79_IndirectSpeechActs}. The responses in \citeA{Clark79_IndirectSpeechActs} were originally divided into three categories: literal answer only $(a_l)$, information only $(a_i)$, or both literal answer \emph{and} information $(a_b)$. For the purposes of this case study, we collapse these response rates into just two categories -- literal answers and informational answers -- by adding the ``both'' counts to each remaining category: $a_i' = a_i + a_b, a_l' = a_l + a_b$. 
...
The prior $P(a)$ over answer utterances is a (truncated) geometric distribution. Let $\ell(a)$ be the length of an answer (e.g. $\ell(\textrm{`cafe 1'})~=~1, \ell(\textrm{`cafe 3 and cafe 4'})~=~2$). Then the probability of that answer is given by:

$$P(a) = \frac{(1 - p)^{\ell(a)}p}{{4 \choose (\ell(a))}}$$

\todo[inline]{rdh: need to work how to express the truncation. basically, we need to place all the probability weight that would be assigned to $\{a : \ell(a)\in [4, \infty)\} $ on $a = \textrm{`1,2,3,4'}$}
We normalize by the number of different answers with the given length, ignoring order. This naturally gives longer answers lower probability, reflecting their higher cost of utterance. In our computational experiment, we fix $p = 0.5$. 
