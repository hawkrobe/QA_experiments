...
\right.
$$
where $\floor{w}$ returns the \emph{floor} of an integer and all operations are assumed to act on the minutes component of a time $w$. For example, the goal $g_{3:50}$ with the threshold set at 3:50 corresponds to a function that rounds input worlds earlier than 3:50 and does not round worlds later than 3:50. This would be the case if the agent knew they would need to leave around 3:50 in order to make it to an appointment on time. We also include the ``pure'' goal $g_0$ of always wanting to know the exact time regardless of the threshold. 

The goal prior $P(g_\tau)$ assigns high probability ($p = xxx$) to the pure goal $g_0$ of wanting to know the exact time, and divides the remaining probability mass across different threshold goals ($\tau \in \{$ 3:31, 3:32, \dots, 3:59 $\}$) according to the following rule: each goal $g_\tau$ is proportional to the log of its distance (in minutes) to the given appointment time $c$, reflecting the intuition that the questioner will be more likely to be running late, and therefore in need of the exact time, as it approaches the appointment time: 

$$P(g_\tau; c) \propto e^{|\tau - c|}  $$
\todo[inline]{put the real probabilities here: fix the probability of the pure goal to be reasonable}
