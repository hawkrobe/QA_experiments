...
\end{array}
\right.
$$ As \citeA{GibbsBryant08_OptimalRelevance} point out, their participants all wore analog watches, hence precise answers have higher cost. In fact, an earlier study in the same paper showed that in the absence of any context, participants wearing analog watches responded with rounded answers 90\% of the time . 
...
To compare our model's performance against the `early group' and `late group' conditions in \citeA{GibbsBryant08_OptimalRelevance}, we ran two simulations. In both, the context is ``I have an appointment at 4:00.'' In the first, the true world state is 3:34 (the `early' condition); in the second, the true world state is 3:54 (the `late' condition). Our results are shown in Figure \ref{fig:timeExperimentResults} and compared against empirical data from Gibbs Jr. and Bryant \citeyear{GibbsBryant08_OptimalRelevance} in Table \ref{table:gibbsJrExp3}. 
...
\todo[inline]{rdh: Discuss influence of answer prior? 	This is what makes the \emph{quantitative} fit work so well}
\begin{table*}[t!]
\centering
...
\hline
``American Express cards?''     & 1.00 & 0.00 & 0.95 & 0.05 \\
\hline
``Credit cards?''     & 0.73 & 0.27 & 0.73 & 0.27 \\
\hline
``Any kinds of credit cards?''     & 0.23 & 0.76 & 0.73 & 0.27 \\
\end{tabular}
\\[1.5pt]
...
Finally, all goal projections $g \in \mathcal{G}$ correspond to solving a simple problem: finding out, for each card the agent owns, whether the restaurant accepts that card. This family of goals is parametrized by \emph{which} cards the agent actually owns, with a prior $P(g)$ using the same likelihoods that govern the world prior\footnote{This corresponds to an assumption that restaurants accept cards proportional to the percentage of people owning those cards. This assumption was suggested by Clark (1979), but not used in his informal analysis).}. Formally, for the goal $g_{\mathcal{C}}$, where $\mathcal{C}$ is a set of cards owned by the questioner, we define a probabilistic projection on worlds $w$, which maps a world to \texttt{true} (or, equivalently, state 1) with probability proportional to the number of cards contained in both sets. 

$$P(g_\mathcal{C}(w) = 1)  \propto  | \mathcal{C} \cap \{c \in \mathcal{C} : c \textrm{ accepted in } w \}|$$

This is equivalent to giving ``partial credit'' for each goal-relevant state that the answerer correctly conveys.\footnote{If we instead use the stricter, deterministic goal projection $$g_\mathcal{C}(w) = \left\{\begin{array}{rcl} 1 && | \mathcal{C} \cap \{c \in \mathcal{C} : c \textrm{ accepted in } w \}| > 0 \\ 0 && \textrm{otherwise}\end{array}\right.$$ we obtain a similar qualitative pattern, but must use an extremely high rationality parameter of $\alpha > 500$ to obtain quantitative fits. Since we also find the probabilistic projection (i.e. ``Is a random one of my cards accepted?'') more theoretically plausible than this deterministic projection (i.e. ``Is at least one of my cards accepted?''), we use the probabilistic one for our simulations.}

\subsubsection{Results}

For each possible question in the experiment, we tested the likelihood of our pragmatic answerer responding with a literal ``yes''/``no'' vs. an exhaustive list of accepted cards, marginalizing over all possible world states. For quantitative fit, we tuned a single rationality parameter $\alpha$, shared by all model components. For the results presented in Table \ref{table:clark79exp5}, $\alpha = 10$. We find that when the questioner asks ``Do you accept MasterCard?'', the pragmatic answerer gives a literal ``yes''/``no'' answer with probability $p = 0.94$ (compared to $p=1.00$ empirically). Similarly, when the questioner asks ``Do you accept American Express?'', the pragmatic answerer gives a literal answer with probability $p = 0.96$ (also compared to $p=1.00$ empirically). The small difference comes from the relative likelihoods of the different cards in the true world. However, when the questioner asks ``Do you accept credit cards?'', the probability of a literal response drops to $p = 0.73$ (compared to $p=0.73$ empirically). Our model output therefore provides a good fit to the original experiment data, both qualitatively and quantitatively.

To further understand \emph{why} our model behaves this way, we examine the posterior over goals that the pragmatic answerer infers from different questions. \todo[inline]{Put in a qudPosterior plot for both the ``credit cards?'' question and the ``MasterCard'' question, once we've finalized the model.} For the ``MasterCard'' question, the pragmatic answerer reasons that the most likely goal is to find out whether the merchant specifically accepts MasterCard, hence a response of ``yes'' perfectly rules out possible worlds in which the merchant does \emph{not} accept MasterCard, informatively resolving the question. Giving a full, exhaustive list of accepted credit cards is still a valid answer, but has low prior probability; the minimal increase in usefulness is not enough to overcome this low prior. 
...
Each model was run with uniform prior probability over worlds, goals, questions, and answers, and with equal cost for all utterances. For each model, a single optimality parameter, which applied to all agents as described above, was fit to maximize correlation with the data. \todo[inline]{rdh: State briefly what we used as our QUD projections?}
