...
Letting $P(w)$ denote a distribution over the space of possible world states $\mathcal{W}$, we will use the notation $\widehat{P^g}(w)$ to indicate the probability of $g(w)$ under the projected distribution. So 
$$\widehat{P^g}(v) = \int_{\mathcal{W}} \delta_{v=g(w)}P(w)dw$$ which is a distribution over the image of $g$.
