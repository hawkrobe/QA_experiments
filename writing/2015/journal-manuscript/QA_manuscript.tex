...
Each model was run with uniform prior probability over worlds, goals, questions, and answers, and with equal cost for all utterances. The explicit meaning of the question ``Where's the $x$?'' mapped a world $w$ to the set of locations occupied by elements of class $x$. For example, the meaning of ``Where's the pet?'' applied to world $w$ would return a set containing the locations of the dalmatian, poodle, and siamese cat, but not the location of the whale\footnote{This meaning function is somewhat complicated by semantics by the use of the definite article: in formal semantics, the denotation of a noun phrase containing a definite article is only defined if there is a unique, salient object that the noun picks out in the world. In the context of our experiment, there are often multiple objects picked out by the noun, which is potentially problematic. The meaning we use corresponds more closely to the utterance ``Where are the pets?'' than ``Where is the pet?'' when there are multiple objects of type $x$.}. For each model, a single optimality parameter, which applied to all agents as described above, was fit to maximize correlation with the data. 
