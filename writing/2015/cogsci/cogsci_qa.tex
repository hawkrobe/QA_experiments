...
For each model, we tuned these two parameters to maximize the correlation between model and data. This yielded a model-data correlation of $r = 0.86$ for the explicit questioner and correlation of $r = 0.89$ for the pragmatic questioner. Although the pragmatic model has a slightly higher fit, the two models only differ slightly in the magnitude of predictions, not in qualitatively important ways such as the rank ordering of response.
...
Finally, we find that our pragmatic answerer model provides a much better account for our data than the explicit answerer. (Though note that the explicit answerer does have one fewer parameter, since it doesn't depend on a questioner model at all.)
