...
\begin{abstract}
What makes a question useful? What makes an answer appropriate? In this paper we propose a model of question-answer behavior within the Rational Speech Act framework. We formulate rational answerers as choosing an answer to an explicit question such that the informativeness of that answer to an inferred underlying speaker goal is maximized. Conversely, questioners are modeled as having a particular goal, and wanting to ask an explicit question that maximizes the informativeness (about that goal) of the answers they are likely to receive, given a model of the answerer. We test this model on three different case studies: first, a classic study in psycholinguistics showing that answerers' level of informativeness varies with the inferred questioner goal, while keeping the question constant; second, we provide evidence from two experiments embedded in a question-answer reasoning game. For all cases, the most sophisticated model makes the qualitatively correct predictions about the questions and answers that speakers produce.
