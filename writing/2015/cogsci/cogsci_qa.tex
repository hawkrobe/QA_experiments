...
\begin{abstract}
What makes a question useful? What makes an answer appropriate? In this paper we propose a question-answer model within the Rational Speech Act framework that models rational answerers as choosing an answer to an explicit question such that the informativeness of that answer to an inferred underlying speaker goal is maximized. Conversely, questioners are modeled as having a particular goal, and wanting to ask an explicit question that maximizes the informativeness (about that goal) of the answers they are likely to receive, given the proposed answerer model that infers the questioner's goal. We test this model on two different case studies: first, a classic study in psycholinguistics showing that answerers' level of informativeness varies with the inferred questioner goal, while keeping the question constant; second, we provide evidence from a question-answer reasoning game. For both cases, the model makes the qualitatively correct predictions about the questions and answers that speakers produce.
...
A:``Go ahead.''
\end{quote}
In this (real life) example, Q strategically chooses a question that differs from his true interest, avoiding an impolite question, yet manages to signal to A what his interests are; A in turn reasons beyond the overt question and provides an answer that addresses Q's interests.
...
We find that sophisticated pragmatic reasoning is needed to account for the data. This suggests that the purpose of questions in dialogue is to provide cues to the answerer about the questioner's interests; the appropriateness of an answer is the extent to which  it is informative about these interests.
...
% Want to motivate RSA a bit more?
...
%In particular, we compare a pragmatic answerer making inferences about the questioner's goals to two simpler models: one that takes into account only that an answerer wants to be maximally informative with respect to the explicit question asked (without inferring the questioner's underlying decision problem) and one that provides a literal answer to the question (without attempting to be maximally informative).  
We show that models in this family can explain some classic psycholinguistic puzzles about answerer behavior.
...
We assume that a question is an informational goal: a projection from worlds to some aspect of interest. This is equivalent to the more common partition semantics of \citeA{GroenendijkStokhof84_SemanticsOfQuestions}, as can be seen by considering the pre-image of such a projection \red{JD: how can we see this, exactly?} \red{RDH: we say this in the next sentence, right? $q^{-1}(a)$ gives the cell of the partition corresponding to the given aspect of interest\dots}). An answer is then a possible value of the projection function (which picks out an element of the partition via $q^{-1}(a)$).
...
The \textbf{literal answerer} ignores the question and simply chooses answers by trading off prior answer probability  and how well a question-answer pair conveys the true state of the world to an interpreter \red{JD: I thought the point was the question is just ignored? why, then, does it figure in the formula?}:
%
$$P(a | q,w^*) \propto P(a) P(w^* | q, a) $$
%
This is equivalent to the speaker in previous RSA models \red{JD: only if q is ignored, no?}.
...
In order to establish that the proposed model can in principle produce answers that are either over- or under informative with respect to an explicitly asked question via inference about an underlying questioner goal, we apply the model to \citeA{Clark79_IndirectSpeechActs}'s whiskey study. Recall that in this study, liquor merchants were more likely to give over-informative answers (specifying exact price) to the question ``Does a fifth of Jim Beam cost more than \$5?'' in the \emph{underinformative} (``I want to buy some bourbon'') condition  than in the \emph{literal} (``I've got \$5 to spend'') condition. %The purpose of this application is to examine whether the model can produce answers to the same question that vary in informativeness--not because the question does not matter, but because the inferred questioner goal differs across contexts.
...
Our world state is a pair of one of $10$ prices (\$1, \$2, \dots, \$10), and a Boolean indicating whether the merchant takes credit cards. There are three possible goals: learning the exact price of whiskey, learning whether the price is greater than \$5, and learning whether they take credit cards. The set of answers includes exact prices as well as ``yes'' and ``no'', with an answer prior that prefers ``yes''and ``no'' to the exact price answers.
...
\section{Exp.~1: Hierarchical questions and answers}
	...
	\begin{figure*}[t!]
\begin{center}
\includegraphics[scale = .45]{pragmatic_questioner.pdf}
\includegraphics[scale = .45]{pragmatic_answerer.pdf}
\end{center}
\caption{Exp.~1 results, compared with the predictions of the best-performing model  for both questioner and answerer.  The explicit and pragmatic questioner do not differ in this task, but the pragmatic answerer better accounts for the qualitative patterns in the response data than the explicit answerer.}
\label{fig:exp1res}
\end{figure*}

 In order to simultaneously test how questioners choose questions when faced with a particular goal and how answerers respond  under uncertainty about this goal, we designed a guessing-game task played by two players: a questioner and an answerer. In this game, $4$ animals (a dalmatian, a poodle, a siamese cat, and a whale) were hidden behind $4$ gates. Note that these animals correspond to different levels in a class hierarchy (see Fig. \ref{fig:taskhierarchy}). The questioner was given a private goal of finding one of the objects (e.g. `find the poodle'), and the answerer (but not questioner) knew the location of each object. Before choosing a gate, the questioner could ask the answerer a single question, chosen from a restricted set of options, and the answerer could respond to this question by revealing the object behind a single gate. 
...
\begin{figure}[t!]
\begin{center}
\includegraphics[scale=.5]{model_fits_grid.pdf}
\end{center}
\caption{Full space of models, and their correlations with the data from Experiment 1. The questioner models in the first row reason about the answerers directly below them in the top row, and the pragmatic answerer reasons about the explicit questioner. Note that the explicit answerer model was fit using one rationality parameter and all others were fit with two parameters: one for the answerer agent and one for the questioner.}
...
First, we are able to rule out both our literal answerer and literal questioner. The \emph{literal answerer} yields a uniform distribution over the four answers that are the case in the given world. This has important consequences for the corresponding \emph{literal questioner} model: when this questioner reasons about which question would generate the most helpful answer from the literal answerer, they find no differences in response probabilities, and therefore have no preference over which question to ask. The predictions of this model, plotted against our empirical results, are shown in the left-hand column of Fig. \ref{fig:model_space}. Based on the results presented above, this is clearly not how questioners and answerers behave and we will not consider the literal models further.

Next, we see that our two remaining questioner models make roughly the same predictions for this task, and we are not able to distinguish them on the basis of these data. 
...
For each model, we tuned these two parameters to maximize the correlation between model and data. This yielded a model-data correlation of $r = 0.96$ for the explicit questioner and correlation of $r = 0.99$ for the pragmatic questioner. Although the pragmatic model has a slightly better fit, the two models only differ slightly in the magnitude of predictions, not in qualitatively important ways such as the rank ordering of response.

The best-fitting pragmatic questioner model's predictions for each response distribution are shown in Fig. \ref{fig:exp1res}(a). Although the magnitude of its predictions are not in perfect alignment with the magnitude of the data (specifically, because it is strongly optimizing), we see that it captures most of the interesting qualitative patterns of the data, particularly the modal responses. 

Finally, we find that the pragmatic answerer model provides a much better fit to the data than the explicit answerer. To ensure equal footing for a model comparison here, we restricted both models to a single optimality parameter that is shared by all agents within the recursion.
...
Only the pragmatic answerer can account for essential qualitative features of the response data. In particular, the explicit answerer predicts that participants will be equally likely to show the `dalmatian,' `poodle,' and `siamese cat' when asked about a mammal. Instead, the data show a significant preference for revealing the cat, leaving `dalmatian' and `poodle' at the same level as the other alternative. The pragmatic answerer correctly predicts this pattern  (as seen in Fig. \ref{fig:exp1res}(b)). Even more dramatically, the explicit answerer predicts a uniform distribution over responses to the `animal?' question (since all four responses indeed denote animals). However, as shown by the $\chi^2$ tests above, the empirical distribution was significantly different from uniform. Thus, the pragmatic answerer is necessary to account for these data.

These data provide strong evidence for a pragmatic answerer, but are much more equivocal with respect the explicit and pragmatic questioner. Because the two models did not make significantly different predictions for the experiment (and both work quite well), we ran a follow-up study on a special case of our guessing-game paradigm in which the explicit and pragmatic questioners make different predictions.
...
\subsubsection{Stimuli \& Procedure} The procedure was the same as before with some changes to the stimuli. The world space $\mathcal{W}$ consisted of possible assignments of the three pets (the dalmatian, the poodle, and the siamese cat) to three gates. The possible goals $\mathcal{G}$ were the dalmatian and poodle (not the cat). The possible questions $\mathcal{G}$ were `dalmatian?' or `cat?'.  The possible answers $\mathcal{A}$ were the three gates. Each participant was given the two goals in a random order. 

\subsubsection{Results}

We found that overall, when the goal was to find the dalmatian, participants were significantly more likely to ask about the dalmatian than the cat, $\chi^2(1) = 12, p < 0.001$. When the goal was to find the poodle, participants were marginally more likely to ask about the cat than the dalmatian, $\chi^2(1) = 3.6, p = 0.058$. When we looked at only the first of the two trials, the dalmatian result held, $\chi^2(1) = 14.4, p < 0.001$ but the participants' preference for asking about the cat disappeared, $\chi^2(1) = 0.07, p = 0.79$. These results are shown in Fig.~\ref{fig:exp2res}.

\subsubsection{Model comparison}

Our explicit questioner model predicted that participants should have no preference for which question to ask given the `poodle' goal, since an explicit answerer would be equally unlikely to give the desired answer for both. The pragmatic questioner model, however, predicts that participants should prefer to ask about the cat. This is because the (internal) pragmatic answerer would reason that if the questioner was interested in the dalmatian, they would ask about the dalmatian; if they didn't, they must be interested in the other possible goal. 

Again, our data are equivocal. Overall, the response distribution matches the predictions of the pragmatic model: questioners prefer to ask about the cat. However, we know from our model that if the questioner does not take into account the full space of possible goals, they will not reason accurately about the pragmatic answerer and will produce a very different response distribution. Participants who received the `poodle' goal on the first trial appeared to behave more like explicit questioners, but this could be explained as an informational asymmetry: if they thought the poodle was the only goal (counter to the instructions), then asking about the dog would be consistent with the pragmatic model as well. We discuss other examples of sensitivity to the presentation of information below.
...
\red{this discussion is good -- we should make sure in final edits that it touches on key points form the intro. e.g. mention how this takes a step toward extending RSA to dialogs...?}

Humans are experts at inferring the intentions of other agents from their actions \cite{TomaselloCarpenter___Moll05_IntentionsCulturalCognition}. Given simple motion cues, for example, we are able to reliably discern high-level goals such as chasing, fighting, courting, or playing \cite{BarrettToddMillerBlythe05_IntentionFromMotionCues, HeiderSimmel44_ApparentBehavior}. Experiments in psycholinguistics have shown that this expertise extends to speech acts as well.  Behind every question lies some goal or intention. This could be an intention to obtain an explicit piece of information (``Where can I get a newspaper?''), signal some common ground (``Did you see the game last night?''), test the answerer's knowledge (``If I add these numbers together, what do I get?''), politely request the audience to take some action (``Could you pass the salt?''), or just to make open-ended small talk (``How was your weekend?''). These wildly different intentions seem to warrant different kinds of answers, even if the explicit question is expressed using the same words. By formalizing the computational process by which answerers infer these different intentions, our model framework provides a unifying way to accommodate this diversity. 

In particular, we have presented computational-level evidence that answerer behavior is best described by a pragmatic model that reasons about questioner intentions, using the question utterance as a signal. The superiority of the pragmatic answerer behavior was robust across all conditions. Questioner behavior, however, seemed to be much more dependent on experience and instructions. For example, in another version of the experiment, we did not focus the questioner's attention on particular aspects of the game such as the fact that the answerer knows about the restricted answer set and the fact that it might be helpful to think about what someone might think when hearing your question (prompting perspective-taking). These instructions reflect essential informational assumptions about the model and without these instructions, questioner behavior shifted more toward that predicted by the explicit model. Indeed, we suggest that our sample contained a mixture of explicit and pragmatic questioners. This may be a product of our artificial game paradigm, or it may be reflective of real tendencies in language use.
...
\end{center}
\caption{Results for experiment 2. The critical test of our models is whether or not questioners prefer to ask about the `cat' or the `dalmatian' when looking for the `poodle'.  In the left panel, we have the overall response distribution, where participants significantly prefer `cat.' In the right panel, we split out this distribution into participants who saw the given goal on the first trial vs. the second trial, showing that there is no significant preference for the two on the first trial.}
\label{fig:exp2res}
\end{figure}


While the artificiality of our experiment design seems to distance the behavior of participants from the types of questions and answers they would make using natural language, there are some benefits to this design as well. Most importantly, it is worth observing that many conversational scenarios in everyday usage feature natural restrictions on the set of things one can ask about, due to politeness, salience, time cost, and other factors. In future work, we will explore the extent to which the proposed model can scale up to more naturalistic language settings.
%We must choose our questions carefully. 
\red{JD: if you leave my inserted sentence, this last sentence needs to go. i think the inserted sentence is useful especially for people who are very skeptical of this sort of work that pretty obviously has fairly low ecological validity.}
\red{RDH: agreed!}

\red{finish with a big-picture sentence or two.}
