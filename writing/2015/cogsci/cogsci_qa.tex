...
\begin{abstract}
\red{edit} What makes a question useful? What makes an answer appropriate? In this paper we propose a model of question-answer behavior within the Rational Speech Act framework. We formulate rational answerers as choosing an answer to an explicit question such that the informativeness of that answer to an inferred underlying speaker goal is maximized. Conversely, questioners are modeled as having a particular goal, and wanting to ask an explicit question that maximizes the informativeness (about that goal) of the answers they are likely to receive, given a model of the answerer. We test this model on two different case studies: first, a classic study in psycholinguistics showing that answerers' level of informativeness varies with the inferred questioner goal, while keeping the question constant; second, we provide evidence from a novel question-answer reasoning game. For both cases, the model makes the qualitatively correct predictions about the questions and answers that speakers produce.
...
While the artificiality of our experiment design seems to distance the behavior of participants from the types of questions and answers they would make using natural language, there are also some benefits to this design. First, it is easy to control the exact space of questions, goals, and answers, which is necessary for a first study of these phenomena. Moreover, while the restrictions on question space may seem peculiar, it is worth observing that many conversational scenarios in everyday usage also feature restrictions on the set of things one can ask about, due to politeness, salience, time cost, and other factors. In future work, we will explore the extent to which the proposed model can scale up to real-time, multiplayer games, extended dialogues, and other more naturalistic language settings. If questions themselves are informative, we must ask them carefully.
