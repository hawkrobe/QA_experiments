...
\begin{abstract}
What makes a question useful? What makes an answer appropriate? In this paper, we formulate a family of increasingly  sophisticated models of question-answer behavior within the Rational Speech Act framework. We compare these models based on three different pieces of evidence: first, we demonstrate how the answerer models handle a classic study in psycholinguistics showing that an answerer's level of informativeness varies with the inferred questioner goal, while keeping the question constant. Second, we jointly test the question and answer components based on empirical evidence from  a question-answer reasoning game. Third, we examine a special case of this game to further distinguish among the questioner models. Ultimately, we find that the most pragmatically sophisticated agents are needed to account for these data, although questioner behavior is more sensitive to informational assumptions (\red{fix me}). These models provide a basis for understanding the rich interplay of questioner signaling and answerer inference in dialogue.
