...
We now present a computational-level analysis of the preceding experiments, comparing several progressively more complex models in the RSA family specified above, eventually determining that a pragmatic answerer is necessary. At the very simplest level, we have an answerer who ignores the question and simply provides an answer that is true, yielding a uniform distribution over the four answers that are the case in the given world. This has important consequences for the questioner: when they reason about which question would generate the most helpful answer, they find no difference, and therefore have no preference over which question to ask. This is clearly not the case and we will not consider it further.

At a slightly higher level of sophistication, we have an answerer that attempts to be informative with respect to the explicit question, without reasoning about the questioner's underlying goals. This answerer up-weights answers that result in a distribution of worlds (under the literal listener) where the leaf nodes picked out by the question are in the correct location. It down-weights answers that would mislead the questioner into the thinking that the leaf nodes mentioned in the question are in the wrong locations. For example, if the questioner asked about the `dalmatian' and the answer replied with the location of the goldfish, the set of worlds in which the goldfish is in the specified location could have the dalmatian in any of the remaining places. Thus, such an answer to this question would be down-weighted. This model produces the answerer predictions depicted in Figure \ref{fig:exp1pred}(b), which successfully down-weights the least relevant alternatives but cannot break the symmetries between the relevant ones, and is still inadequate for reproducing the data. 

While the answerer data does not match the `explicit answerer' model, it does provide a foundation for a questioner model. When the questioner selects questions that maximize the expected information gain from hearing the explicit answerer's response, it produces the distribution of questions shown in Figure \ref{fig:exp1pred}(a) \dots \red{XXX Will fill in once we have better questioner predictions \textbf{Robert}}
