...
\end{figure*}
\section{Experiment 1: \\ Concept structure in questions and answers}
...
We thus constructed twelve items by crossing four conceptual domains (\emph{animals}, \emph{plants}, \emph{places}, and \emph{artifacts}) with three concept hierarchy structures (\emph{branching}, \emph{overlapping}, and \emph{equivocal}; see Figure \ref{fig:hierarchyStructures}).
...
The different concept hierarchy structures were intended as qualitatively different conditions with dramatically different predictions, while the conceptual domains were intended to support item-level generalizability within each condition.
...
\begin{figure*}[h]
\begin{center}
\includegraphics[scale = .85]{Exp1/QualitativePredictions.pdf}
\end{center}
\caption{\footnotesize  Key qualitative results for answerer models  (top) in branching condition and questioners (bottom) in overlapping condition. Highlighted rows indicate clearest qualitative differences. Labels shown from `animals' domain; counts estimated across all domains.}
...
Human participants, on the other hand, have a strong preference: 94\% provided the location of the ``whale,'' the object that isn't queried by any of the other questions, $\chi^2(3) = 210.8, p < 0.001$ (see Fig. \ref{fig:exp1qualitative}).
...
Next, we turn to a subtler qualitative pattern in the ``overlapping'' condition (see Fig. \ref{fig:hierarchyStructures}), which was specifically designed to distinguish between our \emph{questioner} models. 
...
Our non-pragmatic models cannot account for key qualitative phenomena while our pragmatic models ($A_2$ and $Q_2$) make the correct predictions. 
Next, we show that our pragmatic models also provide better overall quantitative fits to the raw behavioral data. 
Before describing the results of our model comparison, we formalize the task in our modeling framework. 
...
Results are shown as marginal likelihoods in Fig. \ref{tab:exp1likelihoods}. 
...
In Experiment 1, we presented both answerer and questioner models with the  \emph{quantitative} challenge of accounting for trial-level variance in a Hidden Goal task. 
...
This is especially striking in the `equivocal' condition where our models predict no preference in questioner behavior: if asking about the goldfish, which is both a ``pet'' and a ``fish,'' the symmetry of the alternatives provides no pragmatic reason to favor one label over the other. 
...
\begin{figure*}[tbh!]
\begin{center}
\includegraphics[scale = .5]{cardsQuestionerResultsWithGraphics.pdf}
\end{center}
\caption{Posterior predictives for each model, which indicate good overall fit (excluding $A_0$ and $Q_0$). Blue dots on $Q$ plots indicate overlapping condition.}
%\vspace{-1cm}
\label{fig:exp1predictives}
...
\section{Experiment 3: Multi-round dialogue}


\begin{figure*}[tbh!]
\begin{center}
\includegraphics[scale = .7]{spatialDemo.pdf}
\end{center}
\caption{Posterior predictives for each model, which indicate good overall fit (excluding $A_0$ and $Q_0$). Blue dots on $Q$ plots indicate overlapping condition.}
%\vspace{-1cm}
\label{fig:exp1predictives}
...
\begin{figure*}[tbh!]
\begin{center}
\includegraphics[scale = .7]{spatialGoalInference_final.pdf}
\end{center}
\caption{Posterior predictives for each model, which indicate good overall fit (excluding $A_0$ and $Q_0$). Blue dots on $Q$ plots indicate overlapping condition.}
%\vspace{-1cm}
\label{fig:exp1predictives}
...
\begin{figure*}[tbh!]
\begin{center}
\includegraphics[scale = .7]{spatialQualitativeQuestioner.pdf}
\end{center}
\caption{Posterior predictives for each model, which indicate good overall fit (excluding $A_0$ and $Q_0$). Blue dots on $Q$ plots indicate overlapping condition.}
%\vspace{-1cm}
\label{fig:exp1predictives}
