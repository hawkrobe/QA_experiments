...
More specific \emph{wh}-questions like ``Who passed the examination?'' may also receive different answers in different contexts. 
...
Taken together, this body of work suggests that \emph{goal-relevance} is a critical feature of question-answering: answerers routinely go beyond the literal meaning of a question to help the questioner achieve their underlying goals. 
The bulk of prior work on goal-sensitivity in inquiry has focused on the \emph{answerer}, holding the question constant and investigating the effect of different contexts, so the extent to which the \emph{questioner}'s choice is affected by social context remains unclear. 
...
A more promising thread of formal work on goal-sensitivity in inquiry has been developed in the linguistics literature on questions \cite<see>[for a thorough review of this literature]{ciardelli2018inquisitive}. 
It is a fundamental problem in formal semantics to precisely define the meaning of questions and answers. 
Early approaches simply identified the meaning of a question with a set of possible answers  \cite{hamblin1973questions,Karttunen77_SyntaxSemanticsQuestions}.
Later work by \citeA{GroenendijkStokhof84_SemanticsOfQuestions} pointed out several limitations of these naive approaches and  formulated an elegant account where that questions have a \emph{single} exhaustive answer in each world. 
Because each world has a unique answer, questions induce a \emph{partition} over the space of possible worlds, where each cell of the partition corresponds to a different answer and all worlds in a particular cell have the same answer.
...
While these accounts grapple with subtle semantic phenomena, the meanings they assign are entirely context-independent. 
A question like ``Where are you?'' will always induce the same partition, demanding the same answers.
Several accounts have argued that the `goodness' of an answer ought to be defined relative to questioner goals  \cite<e.g>{Ginzburg95_ResolvingQuestions,AsherLascarides98_QuestionsInDialogue} but the most influential formal model is due to \citeA{VanRooy03_QuestioningDecisionProblems}.
This account proposes that people ask questions to resolve \emph{decision problems}, so the utility of an answer ought to be evaluated \emph{with respect to that decision problem}.  
In a decision-theoretic framework similar to that used in OED models, the questioner therefore chooses the question that maximizes their expected information gain, assuming that the answerer is cooperative and will choose goal-relevant answers.

\todo[inline]{Revise this to say what the gap is that we're addressing, see rebuttal}
He considers two proposals for how meanings depend on this decision problem, ultimately arguing for the second. 
