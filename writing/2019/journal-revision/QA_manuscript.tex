...
P(g_{ij} | q) & \propto & P(q | g_{ij})P(g_{ij}) \\
 & = & P(q | g_i)P(g_i) \times P(q | g_i^{(j)}) P(g_i^{(j)} | g_i)
 \end{array}
 $$
This inference factors into an ultimate goal term and subgoal term, given two assumptions.
First, that the subgoal $g_i^{(j)}$ depends only on the ultimate goal $g_i$, and \emph{second}, that the likelihood of the questioner asking a question $q$ under a tuple is the product of the likelihoods under its component goals.
This is equivalent to assuming that the questioner sums the utilities of the different goals in their tuple to obtain a composite utility.
A question will be preferred if it is expected to be informative for both the subgoal \emph{and} ultimate goal.
...
Given this posterior over the goal tuple $g_{ij}$, we can then generalize Equation \ref{eq:A2utility} by assuming the answerer attempts to be informative with respect to each goal in the tuple:
$$U_{A_2}(a; q, w) = \alpha_A\E{P(g_{ij}|q)}{\sum_{g\in \{g_i, g_i^{(j)}\}}\ln \widehat{P^g_I}(w|\,a)} - \beta C(a)$$
In other words, the goal tuple induces a tuple of relevance projections, and the answerer averages over them when choosing a response.
...
By directly eliciting the answerer's beliefs about the questioner's goal, we showed that the $A_2$ model's internal goal inference mechanism is required to account for the dynamics of how these beliefs shift over successive questions.
For this reason, the $A_2$ model also accounted for the answerer's behavior substantially better than the $A_1$ model.
