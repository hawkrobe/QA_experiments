...
While introducing such a planning mechanism into our model is outside the scope of this paper\footnote{Optimal policies based on global planning, compared to locally `greedy' policies, are not yet well-understood even in the simpler non-pragmatic case of querying an oracle, though see \citeA{nelson_meder_jones_2018} for recent theoretical progress.}, our final experiment aims to set a foundation for future work by (1) introducing a notion of persistent state into the model and (2) investigating several pragmatic phenomena that emerge from iterating our model forward in time in a step-wise manner.
...
The $A_2$ model successfully accounted for the dynamics of how these beliefs shift over successive questions, and thus was substantially preferred over the $A_1$ model in a quantitative model comparison.
Next, we examined cases where the answer to one question affects the questioner's subsequent choice of question.
Given their representation of the state, both the $Q_1$ and $Q_2$ models accurately predicted the dynamics of questioner behavior over the sequential exchange.

These results show that extending our framework with a persistent state is sufficient to explain some key features of sequential exchanges.
...
An important step for future work is thus to work out an appropriate planning mechanism for updating beliefs about \emph{goals} using question utterances.

Another challenge for future work concerns the structure of goals and subgoals.
In Exp. 2 and 3, we assumed that subgoals were \emph{conditionally independent} given their parent in the hierarchy. 
...
However, this assumption is clearly violated in certain real-world cases.
For instance, sequential dependencies are introduced when subgoals unfold in time.
...
Conversely, answerers may take advantage of schematic knowledge of this sequential structure to anticipate and address subsequent subgoals \cite{jackendoff2007language}.
