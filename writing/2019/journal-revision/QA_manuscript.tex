...
We begin from the observation that question-answer dialogue is motivated by a fundamental asymmetry in beliefs: questioners have a private goal but lack goal-relevant information about the world, and answerers have private information but lack knowledge about the questioner's goal.
...
Critically, we predict that pragmatic answerers go beyond the literal meaning of the question to be informative with respect to inferred goals, and that pragmatic questioners expecting answerers to do so may select questions to more unambiguously signal their goals.
...
We evaluate our pragmatic models against optimal but non-social models in two ways.
First, we present computational simulations accounting for three classic answerer effects in psycholinguistics.
We then introduce the Hidden Goal paradigm for experimentally eliciting both questioner and answerer behavior in scenarios where there is uncertainty about the questioner's goal. 
...
Similar to OED models, it can be shown that a rational questioner should choose the question yielding the maximal expected increase in payoff for the decision problem they are facing.
An answerer agent can similarly be formulated, who selects the maximally useful answer \emph{with respect to that decision problem}.
Because both questioner and answerer expect one another to behave rationally, the questioner can assume their partner will take their decision-problem into account when interpreting their question.
...
However, our account departs from this proposal in addressing a critical limitation it faces as a cognitive model of inquiry: how does the answerer know what the decision problem is? 
\citeA{VanRooy03_QuestioningDecisionProblems} and subsequent accounts in this tradition \cite<e.g.>{benz2006utility} assume that the decision problem is \emph{already mutual knowledge shared by both questioner and answerer}: ``On the assumption that the answerer knows what the agent’s decision problem is, the coarser-grained, and thus partial, answer is [...] preferred.'' (p. 738).
...
We will suggest that the necessary mechanism is (Bayesian) social inference: the answerer inverts a model of the questioner to reason about their likely goals, conditioned on their utterance.
...
\todo[inline]{Address concern about robustness to alternative sets of questions or goals here}
...
This distribution may depend on distinctive visual properties, but on the typicality of the object for the category \cite{Rosch75}, world knowledge about which labels people tend to use to refer to different objects, and other information in common ground \cite{ClarkMarshall1981}.
...
Our framework draws heavily from classical accounts of question and answer semantics in linguistics and the philosophy of language. 
Our development of these ideas, however, leads to a subtly different perspective on the meaning of a question. 
These differences stem from two psychologically meaningful distinctions at the heart of our model.
The first disentangles the literal question meaning from any particular set of possible answers and instead associates it with a specific goal.
The second disentangles the space of latent goals in the questioner's mind from the communicative signal they choose to provide.

On the first point, our framework resonates with dynamic approaches to question meaning \cite<e.g>{Groenendijk99_LogicOfInterrogation}, as well as recent proposals for a broader inquisitive semantics \cite{ciardelli2018inquisitive}, where the formal content of a question is simply the ``issue'' it raises into the context.
These approaches are implemented in a formal logic that distinguishes this notion of inquisitive content from standard informational content.
Defining literal question semantics directly in terms of their relevance or resolution conditions imposes a crucial \emph{modularity} at the semantic level of the model, freeing higher-level pragmatic considerations to handle the problem of how questioners actually think their partner will generate (possible) answers. 
The distribution of likely answers may or may not correspond to the resolution conditions of the literal meaning depending on the level of recursion.

The observation in inquisitive semantics is that declarative sentences may \emph{also} have inquisitive content, providing a unified semantics rather than separate notions of meaning for interrogative and declarative utterances.
In our framework, we made the simplifying assumption that interrogative sentences shift the hearer's beliefs about the questioner's private goals (corresponding to purely inquisitive content) while declarative sentences shift beliefs about the state of the world (corresponding to purely informational content). 
The RSA framework, however, also allows declarative sentences to have as prior work modeling hyperbole in terms of its `inquisitive' 
 the problem of how questioners actually think their partner will generate answers for 
While we do not fully flesh out these connections given the rather large scope of inquisitive semantics (e.g. how classic problems about embedded questions, exhaustivity, and so on are handled), we see this as an interesting area for future work that we hope will resonate with formal semanticists.
...
On the second point, rather than following \citeA{VanRooy03_QuestioningDecisionProblems} in stipulating a context-sensitive literal meaning for questions, we \emph{derive} context-sensitivity at the \emph{pragmatic} level via recursive reasoning on top of context-independent meanings.
%Because different goals are consistent with the same question, 
%The key innovation that allows for this derivation is the psychologically meaningful distinction between the latent goal in the questioner's mind and 

Because a goal is defined to be the same type of object as a classic partition-based question meaning \cite{GroenendijkStokhof84_SemanticsOfQuestions}, we can start with a simple answerer who identifies the goal directly from the question and derive a pragmatic answerer who reasons about \emph{why} a questioner would be asking this particular question, considering the space of possible goals that might lead to the same question.

Our probabilistic account can also be considered an ``update semantics,'' 
...
\todo[inline]{ugh, fix this...}
Because we define the meaning of a question to be the same kind of object as a goal, our $Q_2$ model predicts that people will ask the question most directly corresponding to their goal if it accessible and easy to produce.
...
To generalize beyond the hand-picked sets of alternative questions and answers used in our models above, we may need to consider conditional frequencies from past interactions or simple deletions and edits from the given question \cite<e.g.>{GibsonBergenPiantadosi13_RationalIntegrationNoisy}. To generalize to dialogues lasting longer than a single exchange, we must specify the way in which the contributions of questioner and answerer affect the \emph{context} in which later utterances operate, and the longer-term planning needed for agents to reason effectively about such interactions.

\todo[inline]{ say briefly that taking the exhaustive expectation over all possible worlds does not scale well with the size of the world space...}
