...
The simpler goal spaces considered earlier in the paper are a special case of this more general formulation where the goal tuples are singletons.
...
Here, both $A_1$ and $A_2$ make the same prediction: that participants will reveal the single card that was asked about with high probability and, with lower probability, they may be ``overinformative'' by revealing both the card that was asked about along with another card.
The extent to which the models predict such overinformative responses depends on a ``cost'' parameter penalizing answer length.

In the \emph{baseline} and \emph{overlapping} conditions with hierarchical goals, however, $A_1$ and $A_2$ make different predictions. 
$A_1$ treats these conditions the same as the catch trial, since it attempts to be informative with respect to the goal inherent in the question, and all questions are about a single card.
