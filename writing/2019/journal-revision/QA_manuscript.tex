...
Because there is only one card that must be found to complete the hand, questioners ought to ask about this card.
If they instead asked about a card from one of the possible but un-assigned goals, or a random card not even in the game, we took this as a grounds for exclusion due to inattentiveness.
...
We formalize the notion of a goal hierarchy by extending the goal space $\mathcal{G}$ from a set of possible goal projections $\{g_i\}_{i=1}^k$ to a set of tuples $\{(g_i, g_i^{(j)})\}_{i=1}^k$ where $g_i$ represents an ultimate goal and $g_i^{(j)}$ represents a subgoal of $g_i$. 
In the case of our task, $g_i$ represents the goal of finding an entire combo $i$ --- the projection from the world $w$ to the exhaustive list of locations of all cards in the combo --- while $g_i^{(j)}$ represents the simpler goal of finding just the single $j$th card in the $i$th combo. 
We assume that the questioner attempts to maximize information gain under the ultimate goal $g_i$ of finding their assigned combo.
