...
The present work synthesizes ideas from accounts of questioning in linguistics and psychology to present a unified computational framework addressing these challenges.
...
This provides an alternative view of the function of a question: not as a request for information, but as a signal about private goals.
...
in the five dollar condition, where the goal was simply to find out whether or not the whiskey was \emph{affordable}, a yes/no answer would suffice. 
...
Taken together, this evidence suggests that \emph{goal-relevance} is a critical feature of question-answering: answerers routinely go beyond the literal meaning of a question to help the questioner achieve their underlying goals. 
his work has focused entirely on the \emph{answerer}, with the experimenter holding the question constant and investigating the effect of different contexts. 
Empirical evidence about how social context affects the \emph{questioner}'s choice is relatively lacking; this is the primary motivation for introducing our Hidden Goal paradigm and presenting new empirical data below.
Next, we turn to a set of formal proposals about how .  

\subsection{Formal approaches to goal-sensitivity in inquiry}
...
While \cite{GroenendijkStokhof84_SemanticsOfQuestions} and subsequent refinments grapple with subtle semantic phenomena, the meanings they assign are context-independent. 
A question like ``Where are you?'' will, by definition, induce the same partition, demanding the same answers.
Several accounts have addressed this limitation by arguing that the `goodness' of an answer for resolving a question ought to be defined relative to the questioner's current goals  \cite<e.g>{Ginzburg95_ResolvingQuestions,AsherLascarides98_QuestionsInDialogue} but the most influential formal proposal is due to \citeA{VanRooy03_QuestioningDecisionProblems}.

In a decision-theoretic framework similar to the one used in OED models, \citeA{VanRooy03_QuestioningDecisionProblems} assumes that people will select the question yielding the maximal expected increase in payoff for the decision problem they are facing.
It follows that the usefulness of an answer ought to be evaluated \emph{with respect to that decision problem}.
Given this setup, \citeA{VanRooy03_QuestioningDecisionProblems} argues further that the literal meaning of a question should be taken to be ambiguous or underspecified and this ambiguity is only resolved for the answerer given the contextual parameter of a specific decision problem. 
...
We take this goal-centric proposal as a starting point for approaching context-sensitivity in our social inference framework below.
However, our account departs from \cite{VanRooy03_QuestioningDecisionProblems} in two critical ways, motivated by two limitations as a cognitive model of inquiry.
First, \citeA{VanRooy03_QuestioningDecisionProblems} and subsequent accounts in this tradition \cite{benz2006utility} assume that the decision problem determining relevance is \emph{already mutual knowledge shared by both questioner and answerer}: ``On the assumption that the answerer knows what the agent’s decision problem is, the coarser-grained, and thus partial, answer is [...] preferred.'' (p. 738).
In our view, this assumption contradicts the functional conditions motivating inquiry. 
If the decision problem were already clear, there would be no need to ask a question -- the answerer could simply provide the needed information.
We suggest that the 

The only work we are aware of that relaxes this assumption and considers how the answerer to consider a space of alternative decision problems is \citeA{stevens2014indirect} which only treats a limited special case from a game-theoretic perspective. 
So, one way of framing our theoretical contribution for a linguistics audience is to say that we are introducing the necessary inferential machinery required to extend Van Rooy (2003) to the more general case where answerers have a priori uncertainty over the questioner’s goal, and exploring the broader consequences for pragmatics under such conditions. 

\section{A Rational Speech Act model of \\ asking and answering questions}
...
\todo[inline]{Make note that we sorta derived something like inquisitive semantics from purely projective semantics plus generic gricean mechanism? inquisitive semantics explicitly builds in the goal `raising' as part of the meaning of the question, we only get it at $Q_2$}

\todo[inline]{Tune this up}
\subsection{The meaning of a question}
...
While the relevance projection we used to represent the notion of a goal is equivalent to classic partition-based proposals for the semantics of questions \cite<e.g.>{GroenendijkStokhof84_SemanticsOfQuestions}, %the meaning of the question can technically be described as its set of possible ``answers,'' i.e. the set of states in the image of its projection map, 
the meaning of a question in our framework is conceptually closer to recent proposals for an \emph{inquisitive semantics} specified in formal logic \cite{ciardelli2013inquisitive, roelofsen2013algebraic}.
