...
\title{The role of social reasoning in \\ asking and answering questions}
\shorttitle{Questions and answers in dialogue}
\author{Robert X.D. Hawkins, Noah D. Goodman}
\affiliation{Stanford University} 

\abstract{What makes a question useful? 
What makes an answer helpful? 
Asking questions is one of our most efficient and reliable means of learning about the world. 
Yet we do not often pose these questions to an impartial oracle; we ask cooperative social agents, in dialogue. 
In this paper, we aim to reconcile formal models of optimal question-asking and answering with classic psycholinguistic effects of social context. 
%Agents hear an utterance, update their beliefs through social reasoning, and select an utterance in response. 
We begin with the functional proposal that question-answer dialogue is motivated by a dual asymmetry in beliefs: questioners know their goal but have uncertainty about goal-relevant aspects of the world, while answerers have knowledge about the world but uncertainty about the questioner's goal.
We formalize this asymmetry in a computational framework and derive questioner and answerer behavior from recursive social reasoning.
Critically, this framework predicts that pragmatic answerers should go beyond the literal meaning of the question to be informative with respect to inferred goals, and that pragmatic questioners expecting answerers to do so may select questions to more unambiguously signal their goals.
...
We evaluate our models against optimal but non-social models based on both computational simulations and novel empirical data.
...
\keywords{pragmatics, social reasoning, language, computational modeling, active learning, questions, answers}
...
Additionally, each experiment demonstrates how our core computational framework can be composed and extended with additional elements to capture more sophisticated behaviors, including more complex question semantics, structured hierarchical goals, and multi-round dialogue. 
...
In \cite{Clark79_IndirectSpeechActs}, for instance, researchers called liquor merchants and asked ``Does a fifth of Jim Beam cost more than \$5?'' 
...
Each state is then associated with a conditional belief distribution over the world, $P(w | s_t)$, or about a partner's goals, $P(g | s_t)$. \footnote{This notion of a belief state can in principle be used to define a Markov Decision Process, where different questions or answers actions lead to transitions in beliefs. This connection suggests a future avenue toward embedding our pragmatic agents in a planning framework.}.
Now, when a questioner is selecting a question at time $s_t$, rather than taking expected information gain relative to a general prior $P(w)$ , they it relative to their current beliefs: 

$$$$
Similarly, we assume answerers are attempting to be informative to a questioner with beliefs $P(w | s_t)$
In our experiment, updating beliefs based on the state simply entailed removing any world states from the prior that are incompatible with previous exchanges and re-normalizing.

Aside from this notion of state, this task was formalized similarly to the previous experiments.
Goals were again represented hierarchically, with a joint inference over rows vs. columns and \emph{which} row or column.
The top-level row (column) goals projected worlds (i.e. locations of bombs) to an array of booleans representing which rows (columns) contained bombs and which were safe.
The subgoal representing a particular row (column) projected worlds to a single boolean representing whether that particular row (column) was safe.
\todo[inline]{Finish explaining how worlds/questions/answers were represented}
...
We collapsed across goals for each condition and question by classifying responses as `correct' if matched the true goal and `incorrect' if they differed.
...
\subsubsection{Qualitative predictions and model comparison}

\begin{figure*}[t!]
\begin{center}
\includegraphics[scale = .5]{Exp3/blocked.pdf}
\end{center}
\caption{How behavior unfolded on `blocked' trials condition.}
%\vspace{-1cm}
\label{fig:blocked}
\end{figure*}

Just as the answerer's beliefs about the hidden goal shifted as they heard additional questions, the questioner's preferences over questions ought to shift systematically as they gain additional information about the world. 
To evaluate this prediction, we consider how dialogue unfolded in the \emph{blocked} condition as a qualitative case study.
In the blocked condition, questioners are faced with a setback: while a single safe tile in the initial common ground may make a particular row or column attractive, we ensured that there was a hidden bomb in that row or column.
Answerers in this condition therefore needed to decide how to most helpfully reveal this information, while questioners needed to decide how to proceed in future exchanges given that their initial subgoal failed.
This sets up a situation where questioners may prefer different follow-up questions depending on the answer to their first question, a key test of our model's ability to deal with an updating state. 

In Fig. \ref{fig:blocked}, we show a schematic view of how dialogue unfolded in this condition.
In this example, the questioner was assigned the \emph{row} goal, and 
Answerer predictions computed with $\alpha_Q = 20, \alpha_A = 2.5, \beta = .5$ and questioner predictions computed with $\alpha_Q=13, \alpha_A=2.5, \beta = .5$.
...
\begin{tabular}{@{}llr@{}}
\toprule
\multirow{2}{*}{answerers} & A1 & -1894 \\ \cmidrule(l){2-3} 
 & $A_2$ & \textbf{-1424} \\ \midrule
\multirow{2}{*}{questioners} & Q1 &  \\ \cmidrule(l){2-3} 
 & Q2 &  \\ \bottomrule
\end{tabular}
\end{table}

Finally, while the primary aim of Experiment 3 was to demonstrate how our model framework qualitatively captures repeated question-answer exchanges, these data also provided an additional opportunity to conduct a formal model comparison over a wide variety of items.
Our models had three parameters: a cost weight $\beta_c$ as well as optimality parameters for the questioner and answerer, $\{\alpha_Q$, $\alpha_A\}$.
We placed uniform priors over these three parameters:
$$
\begin{array}{rcl}
\alpha_Q & \sim & \textrm{unif}(0, 20) \\
\alpha_A & \sim & \textrm{unif}(0, 20) \\
\beta_c & \sim & \textrm{unif}(0, 3)
\end{array}
$$
As in Experiment 1, we computed Bayes Factors by evaluating the marginal likelihood of the data under each of our models, integrating over these parameter priors. 
Because the model 

Among the answerer models, we found strong support for $A_2$ over $A_1$

\todo[inline]{Next report some qualitative thing just looking at `blocked' trials? like, how does model predict answerers deal with blockage? what do they reveal instead?}

\todo[inline]{Give a disclaimer about questioner models being very close together again in this task, so full model comparison wasn't super reliable? Still show predictives for absolute fit?}
...
\todo[inline]{Need to say this better}
