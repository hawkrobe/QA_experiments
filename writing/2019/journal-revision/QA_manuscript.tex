...
This agent responded according to the output of our $Q_2$ and $A_2$ models, with the optimality parameter set high ($\alpha = 100$). 
...
\todo[inline]{Finish explaining incorporation of 'state' into model}
The key model innovation required to account for extended sequences is an explicit representation $s_t$ of the \emph{state} at a given time. 
For the purposes of this experiment, $s_t$ can be thought of as the literal board state shared in common ground at time $t$, since all relevant information was maintained visually.
More generally, however, this may represent an agent's general beliefs about the world, or about their partner's goals, at a given time (this is how we are setting a foundation for possible later integration with a POMDP).
Rather than taking expected information gain relative to a general prior, then, we take it relative to the beliefs about the world given the state: 
$$MATH$$
