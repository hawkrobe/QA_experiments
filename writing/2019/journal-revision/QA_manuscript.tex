...
Deciding how to ask and answer questions in the presence of such temporal dependences requires a \emph{planning} mechanism over a persistent state.
A planning mechanism may also be necessary for integrating the pragmatics of question asking and answering into more general framework for dialogue.
...
We recruited 124 participants from Amazon Mechanical Turk. 
21 participants were excluded for disconnecting before the end of the task, leaving 104 participants in our final sample. 
All participants were self-reported native English speakers.

\subsubsection{Stimuli}

On each trial, the questioner was assigned one of two goals: complete any \emph{row} or complete any \emph{column}. 
Because each of these goals can be satisfied in as many ways as there are rows or columns, this induces a hierarchical goal space where different 
In addition to this goal, each trial was defined by two sets: an underlying `map' assigning bombs to locations and an `initial state' indicating which tiles were shown as safe to both players at the outset of the trial.
We sampled trials in five different conditions to create a variety of different scenarios.
In the \emph{empty} condition, no tiles were were
