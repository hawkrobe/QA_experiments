...
\citeA{VanRooy03_QuestioningDecisionProblems} then argues that the literal meaning of a question is underspecified: it only resolves to a particular partition given the contextual parameter of a specific decision problem. 
...
Our computational framework begins with four primitive sets that agents take to be in common ground. 
A particular choice of these sets specifies a model for a concrete scenario.
...
Since our questioner models depend on the answerer model they expect responses to come from, we first introduce a family of answerers.
