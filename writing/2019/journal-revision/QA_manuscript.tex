...
\end{center}
\caption{\footnotesize Task display and procedure for Experiment 1. (A) Questioner is privately assigned a goal object to find, but does not know which objects are behind which doors. (B) Answerer is privately shown the locations of objects but does not know which object the questioner is trying to find. Possible questions are shared. (C) Example of a single trial proceeding through 4 phases: goal assignment, question selection, answer selection, and location guess.}
\label{fig:expviews}
\end{figure*}
...
In Experiment 1, we extend our model with an underlying formal account of demonstrative reference in questions (e.g. ``where is \emph{the} dog?'') which uses graded knowledge of concept typicality.
...
\end{center}
\caption{Example of each type of conceptual hierarchy used in our experiment. The bottom row served as the goal and answer spaces, and the possible questions are highlighted in red. }
...
However, among these labels, only ``animal'' could be referring to a whale.
...
Each item consisted of four objects and four labels, such that different labels referred to subsets of objects at different levels of abstraction. 
...
\begin{figure*}[t!]
\begin{center}
\includegraphics[scale = 1]{Exp1/QualitativePredictions.pdf}
\end{center}
...
After passing a short comprehension quiz on the game instructions, participants randomly assigned to the role of questioner or answerer and paired with another participant in a shared virtual environment using the methodology described in \citeA{Hawkins15_RealTimeWebExperiments}. 
Some aspects of each participant's environment were denoted as private (i.e. the questioner's goal and the answerer's knowledge of object locations) while others were explicitly flagged as common ground (i.e. the sets of possible questions and objects; Fig \ref{fig:expviews}A,B).
...
\todo[inline]{Finish this justification...}
In addition to avoiding ad hoc assumptions about these sets when comparing our models, making the alternatives explicit \dots
...
\todo[inline]{Can we do these with mixed effects models so we don't have to collapse over domains?}
...
\subsection{Enriching question semantics with saliency knowledge}
...
In our interactive behavioral experiment, we presented both answerer and questioner models with the  \emph{quantitative} challenge of accounting for trial-level variance in a reference game. We found strong converging support for $A_2$ relative to our other answerer models and, for the first time, tested the predictions of our questioner models. 
