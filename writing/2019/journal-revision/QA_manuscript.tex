...
We begin from the functional hypothesis that question-answer dialogue is motivated by a two-sided asymmetry in beliefs: questioners have private goals but are uncertain about goal-relevant aspects of the world, while answerers have private knowledge about the world but are uncertain about the questioner's goal.
We formalize this asymmetry in a computational framework and derive pragmatic questioner and answerer behavior from recursive social reasoning.
...
First, we shown computational simulations accounting for classic effects in psycholinguistics.
...
One particularly influential account formalizes question asking as an optimal experiment design (OED) problem \cite{OaksfordChater94_RationalAnalysisSelectionTask,Nelson05_UsefulQuestions,MyungPitt09_OED, GureckisMarkant12_SelfDirectedLearning}, where people choose a question to maximize the knowledge they expect to gain from knowing its answer. 
A central ingredient in this framework is the distribution over possible answers people expect to receive after asking a question.
This distribution is typically assumed to be determined by the ground truth outcomes of the task. 
For instance, in a category learning task \cite{MarkantGureckis14_ActiveLearning}, the possible answers to a query about the category membership of a particular object (``Which category is this?'') are simply the possible categories (``It's a \emph{dax}.'')
In other words, an OED questioner expects answers to come from an oracle that responds truthfully but literally.

While this assumption has been sufficient to capture a wide variety of inquiry phenomena in active learning domains including causal learning \cite{bramley2015conservative} and sequential ``20 questions''-style search problems \cite{RuggeriLombrozo15_ChildrenAdaptQuestions}, we argue that it is violated precisely under the conditions where inquiry is perhaps most prevalent and useful: asking linguistic questions to other social agents, in dialogue \cite{coenen2018asking}.
Answerers regularly go beyond the literal meaning of a question in subtle, context-dependent ways. 
\todo[inline]{Say something about how sometimes it is literal, but often go beyond the }

[How do answerers actually choose among the broad range of possible responses? 
The semantics and pragmatics of questions and answers has received extensive treatment in a largely independent tradition within the field of linguistics.]
...
A, in turn, reasons under uncertainty beyond the overt question and provides an answer that addresses Q's true interests. 
Depending on the circumstances, he can adjust his response to be over- or under-informative with respect to the literal question, or to address a different question altogether.  
This deep interdependence between question and answer poses a challenge for formal accounts of inquiry. 

We suggest, following recent accounts \cite<see>[for a thorough review]{coenen2018asking}, that the value of a natural-language question is the extent to which it can be expected to elicit useful information. 
...
\todo[inline]{Revamp this section as discussed in rebuttal}
