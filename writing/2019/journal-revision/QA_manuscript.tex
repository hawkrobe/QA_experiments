...
This paradigm allows us to rigorously compare our different models, and to empirically justify modeling assumptions.
...
First, while preserving the asymmetry in world knowledge between questioner and answerer used in prior work, it \emph{also} explicitly introduces an asymmetry in knowledge about \emph{goals}.
We privately assign one of several possible goals to the questioner and withhold information about the identity of goal from the answerer, thus inducing uncertainty. 
...
\begin{figure*}[t!]
\begin{center}
\includegraphics[scale = 1]{Exp1/Exp1Task.pdf}
\end{center}
\caption{Task display and procedure for Experiment 1. (A) Questioner is privately assigned a goal object to find, but does not know which objects are behind which doors. (B) Answerer is privately shown the locations of objects but does not know \emph{a priori} which object the questioner is trying to find. Possible questions are shared. (C) Example of a single trial proceeding through 3 phases: goal assignment, question selection, and answer selection.}
\label{fig:expviews}
\end{figure*}
\section{Experiment 1: Conceptual knowledge and saliency}

%%%
\todo[inline]{Introduce motivation behind experiment, i.e. very simple case with restricted spaces...}
Our first experiment is a 
