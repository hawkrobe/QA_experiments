...
The counts of participants asking each possible question in this scenario, collapsed across domains, is shown in Fig. \ref{fig:exp1qualitative}. 
...
\begin{figure*}[tbh!]
\begin{center}
\includegraphics[scale = .7]{Exp2Task.pdf}
\end{center}
...
We constructed the space of goal \emph{combos} on each trial in two different ways, leading to two conditions where our questioner models made qualitatively different predictions.
In the \emph{baseline} condition, we sampled two cards to form one combo and then formed the other by sampling two additional cards from those remaining. 
In this case, both models predict that the questioner should have no preference over which card to ask about.
In the \emph{overlapping} condition, however, one card appeared in both combos along with distinct distractor cards. 
In this condition, our pragmatic model predicts that the questioner will avoid asking about the overlapping card; although it is a necessary subgoal, it would provide an ambiguous signal to a pragmatic answerer about the ultimate goal.
Finally, we constructed catch trials using single-object goals. 
Because there is only one card required, questioners ought to ask about this card.

\subsubsection{Procedure}

Unlike Experiment 1, which used a real-time multi-player design to (truthfully) convince players they were playing with a real partner, we decided in this experiment that the benefit of minimal interaction did not outweigh the loss in control over the input seen by each participant.
Thus, rather than being paired with another partner, after passing a comprehension quiz, participants were paired with a simple artificial partner.

Participants were either assigned to the role of the questioner or answerer and saw each of the three trial types (baseline, overlapping, and catch trials) once in a random sequence before switching roles and repeating the three trial types, for a total of six trials. 
