...
Because these predictions are based purely on the hierarchy structures, we treat the four domains as item-level effects, but continue using examples from the ``animal'' domain for readability.
...
We compared this to an alternative model adding an additional regressor that was dummy-coded 1 for ``whale'' and 0 for the other levels, allowing the model flexibility to break the symmetry.
Because these models are nested, we used a likelihood-ratio test comparison accounting for additional degrees of freedom. 
We found that the null model representing $A_1$ was strongly rejected, $\chi^2(3) = 181, p < 0.001$.
...
\todo[inline]{Revamp this section}
