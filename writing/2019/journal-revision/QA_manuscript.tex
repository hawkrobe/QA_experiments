...
By manipulating whether objects overlapped or differed across sets in the space of possible goals, we constructed scenarios where our pragmatic models made sharply different predictions than prior models. 
As predicted by $A_2$, we found that answerers reasoned beyond the literal subgoal prompted by the question to respond helpfully to the inferred ultimate goal above it in the hierarchy.
...
The minimal goal hierarchy modeled in this experiment is a key first step toward formally addressing more structured spaces of goals in cognitive models of question asking and answering.
However, it is also important to note that handling more sophisticated goal structures will require composing our core computational framework with additional mechanisms.
In particular, the more general form of the model we introduced relies on a simplifying assumption of different subgoals being conditionally independent given their parent in the hierarchy: this assumption justifies representing subgoals as tuples with their parent, as well as the decomposition of the utility such that each subgoal equally contributes to the ultimate goal.

\todo[inline]{Finish this sketch\dots}
This assumption is clearly violated in certain cases.
...
These subgoals unfold in time, creating sequential dependencies are introduced.
If we are in an unfamiliar kitchen, it may be important to ask certain questions before others: knowing \dots

Deciding how to ask and answer questions in the presence of such dependences therefore requires a \emph{planning} mechanism.
While introducing such a planning mechanism is outside the scope of this paper, we \dots 
...
Our final experiment tests the extent to which 
