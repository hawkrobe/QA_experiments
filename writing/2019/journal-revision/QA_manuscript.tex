...
\author{Robert D. Hawkins, Noah D. Goodman}
...
We begin with the simplest possible answerer, $A_0$, who attempts to be informative about the state of the world without considering relevance to his partner's goals. 
...
$$P_{A_0}(a | q, w) \propto \exp\{U_{A_0}(a;w)\}$$ 
For simplicity, we omit the implicit uniform prior over answers $P(a)$, which strictly serves to specify the space of answers considered.

We define the utility of the utterance using the information-theoretic measure of surprisal: the increase in an imagined interlocutor's $I$'s certainty about the true state of the world after hearing $a$. 
This surprisal is traded off against the cost $C(a)$ of producing the answer utterance:
\begin{equation}
\label{eq:A0utility}
...
where $\delta_{e}$ is the delta function returning 1 when $e$ evaluates to true and 0 otherwise, and $P(w)$ is the (shared) prior over states of the world. 
...
%\end{array}
%$$
In the context of answering a question, the simplest assumption about the goal projection is that it is the literal meaning of the question:
%This modified utility suggests simply using the literal meaning of the question $q$ itself as the goal projection:
$$P_{A_1}(a|q,w) \propto \exp\{U_{A_1}(a; q,w)\}$$
...
For example, $A_1$ is capable of giving non-yes/no answers to polar questions to the extent that they resolve the literal goal raised by the question\footnote{Note that having the utility only depend on informativeness under the goal projection can lead to behavior where the goal-relevant answerer is happy to say something literally false as long as it maps to a truthful resolution of the coarser goal. Sometimes this is desired (as when we round the time to the nearest hour), but other times it is unintuitive. In principle, this problem could be solved by introducing the notion of a \emph{goal hierarchy} where the pure identity goal always has some small weight (see Exp.~2 for a formalization of this notion). In our simulations, however, we use a ``hard'' version of the same idea that removes literally false utterances from the possible answer set and then renormalizes the answer prior before computing the utility.}. 
...
$$
\begin{array}{lcl}
P_{Q_i}(q|g)  & \propto & \exp\{U_{Q_i}(q;g)\}\\
...
The value of this answer depends on its \emph{information gain} $IG^g(a,q)$: the gap between the questioner's beliefs about the $g$-relevant state of the world before and after hearing an answer.\footnote{As with our answerer, we omit the implicit uniform prior over questions $P(q)$. A cost term $C(q)$ would be appropriate in the questioner's utility, but for the cases we consider in this paper, the questions of interest have the same length, so we also omit cost for simplicity.} Drawing again upon information theory, we formalize the notion of information gain using the Kullback-Leibler divergence between the $g$-projected world prior and posterior after hearing $a$:
%
$$
...
The above equations specify a questioner for each answerer model. $Q_0$, who plans their question by reasoning about the likely responses of $A_0$, does not prefer any utterance over any other, because they will all lead to the same distribution of (informative but not necessarily relevant) answers. 
$Q_1$, on the other hand, expects $A_1$ to directly interpret her question as a goal and respond informatively and relevantly, thus allowing for some questions to be better than others. 
It therefore instantiates a version of the expected information gain measure of usefulness proposed by Optimal Experiment Design (OED) models \cite{coenen2018asking}. 
Critically, in contrast to classical views \cite<e.g.>{GroenendijkStokhof84_SemanticsOfQuestions} the questioner's behavior is not necessarily governed fully by the semantics of the question she asks, but by what she actually expects her partner to say after hearing it. 
They coincide when reasoning about the simple $A_1$ answerer, but we next turn to a more socially perspicacious answerer where they diverge.
...
\citeA{Clark79_IndirectSpeechActs} found that (1) and (2) were nearly always answered literally, with a `yes' or `no', while (3) was significantly more likely to be answered with full information (a list of all credit cards accepted). Which, if any, of our answerer models show this pattern of responses? % (see Table \ref{table:clark79exp5}). 
...
The latter question $q_{any}(w)  = |w| > 0$ projects to a boolean corresponding to whether any cards are accepted at all. %The prior distribution $P(q)$ over these questions is proportional to question length, which happens to be uniform.

The answer space $\mathcal{A}$ includes lists of cards (e.g. ``the cards we accept are Visa and MasterCard''), which are interpreted exhaustively, and  `yes' and `no.' 
We take the denotations of these polar responses to depend on the literal meaning of the previous question: $a_{\text{yes}}(w;q) = q(w)$ and $a_{\text{no}}(w;q) = \neg q(w)$.
...
$g_{\mathcal{C}_q}(w) = |\mathcal{C}_q \cap w | > 0$.
...
We found that the existence of this alternative does slightly reduce the answerer's preference for responding with the exact price (from 62\% to 60\%, because $A_2$ reasons that if $Q_1$ really cared about the exact price, they would have asked this other question).
However, this alternative has the same effect under both contexts, leaving the qualitative context effect unchanged.

A similar question arises in the credit card scenario: if the most direct question, ``What cards do you accept?'', were a potential alternative, would it interfere with the answerer's sensitivity to the questions they were asked?
To explore this case, we assigned this question a literal meaning that projects the world to the exact list of cards that are accepted and re-computed $A_2$'s behavior.
...
Most conspicuously, there is a relative lack of data on how people \emph{choose questions} in similar social scenarios---when the questioner knows her partner wants to be helpful but may not know ahead of time exactly what goal she is pursing.
...
%%%
As a first quantitative test of our models, we designed a cooperative guessing game that relies on knowledge of ontological relations between concepts.
...
Before formally testing the importance of these empirical priors in our model comparison, we conducted a simple test of the null hypothesis that participants use a \emph{uniform} prior over objects consistent with the noun when interpreting the definite article. 
...
In other words, we assume participants use some mixture between the uniform prior (over objects consistent with the noun) and empirical priors, but we have uncertainty over the mixture weight.
...
Upon hearing a question, $A_2$ must jointly infer the ordered pair $g_{ij} = (g_i, g_i^{(j)})$ containing both the questioner's ultimate goal and their local subgoal:
$$
\begin{array}{rcl}
P(g_{ij} | q) & \propto & P_{Q}(q | g_{ij})P(g_{ij}) \\
\end{array}
$$
If the subgoal $g_i^{(j)}$ depends only on the ultimate goal $g_i$, then the prior factors as $P(g_{ij}) = P(g_i)\cdot P(g_i^{(j)} | g_i)$.
To compute the likelihood $P_Q(q|g_{ij})$ of a particular question $q$ under a tuple of goals, we assume the questioner simply sums the utilities of asking $q$ under the component goals: % inference factors into an ultimate goal term and subgoal term, given two assumptions.
$$U_{Q}(q; g_{ij}) = U_{Q}(q; g_i) + U_Q(q; g_i^{(j)})$$
That is, a question will be preferred if it is expected to be informative for both the subgoal \emph{and} ultimate goal.
...
Given this posterior over the goal tuple $g_{ij}$, we can generalize Equation \ref{eq:A2utility} by assuming the answerer also has a combined utility. That is, she attempts to be informative with respect to each goal in the tuple:
$$U_{A_2}(a; q, w) = \alpha_A\E{P(g_{ij}|q)}{\sum_{g\in \{g_i, g_i^{(j)}\}}\ln \widehat{P^g_I}(w|\,a)} - \beta C(a)$$
%In other words, the goal tuple induces a tuple of relevance projections, and the answerer aggregates over them when choosing a response.
Information relevant to the ultimate goal would already provide information relevant to the subgoal; adding the subgoal explicitly in this utility increases the desire of the answerer to address the current subgoal compared to others (that are still part of the ultimate goal).
While this presentation is written in terms of ordered pairs, it straightforwardly extends to deeper goal hierarchies represented as $n$-tuples. 
The simpler goal spaces considered earlier in the paper are special cases of this more general formulation, where the goal tuples are singletons ($n=1$).
...
Questioners, in turn, systematically chose to ask about the objects that would most clearly signal their ultimate goal to their partner, even though these different questions would have equal expected information gain under OED models. 
...
In particular, deciding how to ask and answer questions in the presence of sequential dependences in the goal space likely requires a \emph{planning} mechanism over a persistent state.
While introducing such a planning mechanism into our model is outside the scope of this paper\footnote{OED policies based on global planning, compared to locally `greedy' policies, are not yet well-understood even in the simpler non-pragmatic case of sequentially querying an oracle, though see \citeA{nelson_meder_jones_2018} for recent theoretical progress.}, our final experiment aims to set a foundation for future work by (1) introducing a notion of persistent state into the model and (2) investigating several pragmatic phenomena that emerge from iterating our model forward in time in a step-wise manner.
...
First, we needed a larger and more natural space of goals that wouldn't always be possible to satisfy in a single round.
...
Each state is then associated with a conditional belief distribution over the world, $P(w | s_t)$.
...
To illustrate our findings, Fig. \ref{fig:blocked} shows the distribution of responses in an example blocked trial.
The initial common ground revealed that the top-left tile was safe, and the questioner's private goal was to complete any row. 
All questioners began by asking about either A2 or A3, the other tiles in the top row.
If they asked about A2, the most frequent empirical answer (62\%) was to confirm that A2 was safe, and then give additional information that A3 was \emph{not} safe.
If they asked about A3, the most frequent empirical answer (52\%) was to confirm that A3 contained a bomb, and then give additional information about a safe tile in a row that was not blocked (e.g. C1 or C2). 

In both cases, answerers went beyond the literal question to provide additional goal-relevant information.
When it was the questioner's turn to ask a second question, they were thus faced with a much different epistemic state.
This updated state suggested pursuing a different subgoal (i.e. a different row) than they were pursuing at the outset. 
\emph{Which} subgoal they pursued on their second question differed depended both on their first question and the answerer's response.
The $Q_2$ and $A_2$ models successfully capture these dependencies between successive exchanges by updating their representations of the state.
...
\end{tabular}
\end{center}
\caption{Results of quantitative model comparison for Exp. 3; maximum likelihoods shown on log scale.}
...
Because evaluating marginal likelihoods over this parameter space was computationally intractable, we use the maximum likelihood as a point estimate of the marginal likelihood.
This is equivalent to an AIC or BIC model comparison approach because the number of parameters is matched across the candidate models.
...
In Exp. 3, we considered sequences of questions and answers that extended beyond a single exchange.
We equipped our models with a persistent representation of the state and tested their predictions in a multi-round Hidden Goal task where the content of earlier exchanges may influence later exchanges.
First, we validated the goal inference mechanism in the $A_2$ model by directly eliciting the answerer's beliefs about the questioner's goal.
The $A_2$ model successfully accounted for the dynamics of how these beliefs shift over successive questions, and was also substantially preferred over the $A_1$ model in a quantitative model comparison.
Next, we examined cases where the answer to one question affects the questioner's subsequent choice of question.
Given a persistent representation of state, both the $Q_1$ and $Q_2$ models accurately predicted the dynamics of questions asked over the sequential exchange.

These results show that extending our framework with a state representation is sufficient to explain some key features of sequential exchanges.
However, we expect that \emph{planning} will be necessary for integrating the pragmatics of question asking and answering into longer dialogue sequences, as it has been for other domains \cite<e.g.>{bramley2015conservative}.
Our proposed extension maintaining a (belief) state could be used to define a Markov Decision Process where different question or answer selections lead to anticipated transitions in beliefs about goals or the world, respectively. 
A related approach was recently taken by \citeA{khani2018planning}, who used a forward-looking recursive utility to define an action policy in a sequential language game.
In this task, pragmatic agents took turns saying one- or two-word utterances, informing their partner about the world state, but were not permitted to ask questions. 
An important step for future work is thus to work out an appropriate planning mechanism for how question utterances are expected to shift beliefs about \emph{goals}.

Another challenge for future work concerns the structure of goals.
The hierarchical `row' and `column' goals in Exp. 3 were more complex than the multi-object goals in Exp. 2\footnote{In fact, while the goals in Exp. 2 are technically hierarchical, a model ignoring the subgoal and only optimizing for ultimate goals would be sufficient to fit the data. This is not the case in Exp. 3, where reasoning about the current subgoal is integral for accurate predictions. We introduced goal hierarchies into the model at an earlier juncture for expositional clarity and consistency with their treatment in Exp. 3.}.
However, both of these experiments satisfied the assumption that subgoals are \emph{conditionally independent} given their parent in the hierarchy. 
...
For instance, sequential dependencies among subgoals are introduced when a process unfolds in time.
%For example, the overarching goal of making coffee decomposes into subgoals of turning on the faucet, boiling water, grinding beans, and so on . 
Certain questions may only be useful once earlier subgoals have been satisfied, leading to more or less optimal sequences of questions.
Conversely, answerers may take advantage of schematic knowledge of this sequential structure to anticipate and address later subgoals \cite{jackendoff2007language}.
...
Once we explicitly address the issue of how the answerer actually \emph{infers} this decision problem from hearing the question (and from other context), it becomes unnecessary to additionally posit an underspecified literal question meaning.\footnote{RSA models have handled vague or underspecified literal meanings by introducing a free variable into the meaning that is determined by the speaker and inferred by the pragmatic listener \cite<e.g>{LassiterGoodman15_AdjectivalVagueness,BergenLevyGoodman16_LexicalUncertainty}. An implementation of van Rooy's proposal would require the free variable in the literal meaning to be derived from the questioner's private goal itself. It is unclear how such a free variable could be inferred without circularity, or whether such a mechanism could lead to appropriate interactions between context and question interpretation.}
...
As a broader point, our framework offers a rational analysis of the social inferences involved in asking and answering questions, which only adds to the many important algorithmic questions already raised by rational models of inquiry \citeA{coenen2018asking}.
...
We used exact enumeration for all of our results for theoretical precision, but it is clear that an algorithmic account must address how these computations can be effectively approximated, perhaps via Monte Carlo integration or approximation by neural networks.
