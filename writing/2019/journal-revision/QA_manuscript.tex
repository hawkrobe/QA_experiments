...
\ndg{given that we use g-projection below, we should probably use it here rather than introducing it in a footnote? on the flip side, we never need a kernel which isn't delta in this paper, so it's better to mention the general case only in footnote.}
