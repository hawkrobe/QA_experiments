...
\begin{figure*}[t]
\begin{center}
\includegraphics[scale=.45]{simulations/clarkExampleFig}
\end{center}
\vspace{-1cm}
...
These considerations may also shed new theoretical light on recent results by \citeA{rothe2018people} finding that in games like Battleship, people fail to ask the maximally informative natural language questions (although they \emph{can} accurately rank the informativity of pre-generated questions). 
By embedding active learning in a social, communicative context \cite<see also>{ShaftoGoodmanFrank12_LearningFromOthers, ShaftoGoodmanGriffiths14_Pedagogical}, our framework suggests that in the typical cooperative setting for asking questions, this behavior may be resource-rational \cite{GriffithsLiederGoodman15_LevelsOfAnalysis}.
If the maximally informative questions to elicit information from a literal ``oracle'' answerer tend to be longer and more syntactically or conceptually complex (e.g. ``At what location is the top left part of the purple ship?''), they may also be more cognitively demanding to generate.
When assuming a cooperative pragmatic answerer, however, a simpler and less literally informative question may be just as effective for signaling the goal.
