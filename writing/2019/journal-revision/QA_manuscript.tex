...
The central construct of a \emph{goal representation} has a long history in psychology \cite{miller1960plans, Schank:1977hh, austin1996goal}.
The simplest spaces of goals, like those used in Experiment 1, are discrete sets of projections on the world state.
For example, in the Stroop task commonly used to study cognitive mechanisms of goal-directed behavior, the goal is either to (1) read words or (2) name the color in which they are written \cite{miller2001integrative}.
...
For example, if we know an agent takes the same route home every day, but one day we observe them veering off of this route toward the grocery store, we infer that they intend to run this errand as a subgoal along the way of their ultimate goal of getting home. 

The hierarchical structure of goals has also long been recognized by linguistics working on discourse and relevance.

We thus designed a simple task, inspired by tasks used in multi-task reinforcement learning \cite{oh2017zero, andreas2017modular}.
Since is at the heart of
