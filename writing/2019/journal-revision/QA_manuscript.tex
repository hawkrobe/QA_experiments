...
Just as the answerer's beliefs about the hidden goal shifted as they heard additional questions, the questioner's behavior ought to shift systematically as they gain additional information about the world. 
In particular, questioners may prefer different follow-up questions depending on the answer to their first question, a key test of our model's ability to deal with an updating state. 
To evaluate this prediction, we consider how dialogue unfolded in the \emph{blocked} condition.
The blocked condition was designed as a case study where questioners are faced with a setback. 
While a single safe tile in the initial common ground may make a particular row or column attractive, we ensured that there was a hidden bomb in that row or column.
...
\begin{figure*}[t!]
\begin{center}
\includegraphics[scale = .5]{Exp3/predictives.pdf}
\end{center}
\caption{The mean probability assigned to participant responses under different models.}
%\vspace{-1cm}
\label{fig:exp3predictives}
\end{figure*}

Among the answerer models, we found strong support for $A_2$ over $A_1$. 
Predictives are shown in Fig. \ref{fig:exp3predictives}.
