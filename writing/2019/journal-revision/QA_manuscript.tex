...
Our framework draws heavily from theoretical treatments of question and answer semantics in linguistics and the philosophy of language. 
...
The first distinction disentangles the question meaning from its possible answers and instead associates it with a particular goal it literally signals.
The second distinction disentangles the space of latent goals in the questioner's mind from the communicative signal they choose to provide about that goal.



rather than following \citeA{VanRooy03_QuestioningDecisionProblems} in stipulating a context-sensitive literal meaning for questions, we \emph{derive} context-sensitivity at the \emph{pragmatic} level via recursive reasoning on top of context-independent meanings.
...
Thus, while the relevance projection we used to represent the notion of a goal is equivalent to classic partition-based proposals for the semantics of questions \cite<e.g.>{}, the meaning of a question in our framework is conceptually closer to recent proposals for an \emph{inquisitive semantics} specified in formal logic \cite{ciardelli2018inquisitive}. % which resonates with ongoing work in this literature.
Unlike previous accounts in the literature, which identify the meaning of a question with its possible answers (whether this is unique exhaustive/relevant answer), both our framework and that of inquisitive semantics end up disentangling the meaning of questions from the space of answers. 
Interrogative sentences under our account simply provide information about goals or issues (QUDs) while declarative sentences provide information about the state of the world. 
