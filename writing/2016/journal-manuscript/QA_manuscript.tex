...
This paper makes two key theoretical contributions. First, while psychologists have learned a lot about questioner behavior and answerer behavior by studying the two processes in isolation, we argue that there are significant theoretical benefits in viewing them as deeply intertwined \emph{social} inferences. Second, we formalize this view in a probabilistic model of pragmatic language understanding, bridging the gap between the simple queries used in psychological research on optimal information search and the richer semantics of natural language questions studied by linguistics. This is the first time that such pragmatic language models have been extended beyond single utterances and hence provides a first step toward modeling longer dialogues using simple planning mechanisms. 
%

The rest of this paper is structured as follows. First, we review previous experimental and formal modeling efforts to situate question-asking and question-answering in a social context. We then lay out the details of our computational framework, formulating several questioner and answerer models embodying different hypotheses. Next, we present a set of computational experiments demonstrating how well these models capture three classic answerer-sensitivity effects, and address a longstanding paucity of data on the context-sensitivity of \emph{questioner} behavior by running a real-time communication experiment. This experiment allows us to distinguish between models by manipulating private goals while simultaneously observing both ends of the dialogue. Finally, we demonstrate how smoothly our framework interfaces with insights from linguistics: incorporating \emph{salience} into the interpretation of the definite article improves predictions on our experimental data.
...
\begin{figure*}[t]
\begin{center}
\includegraphics[scale=.5]{clarkExampleFig}
\end{center}
\vspace{-2cm}
