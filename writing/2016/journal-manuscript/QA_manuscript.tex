...
In Supplemental, we show that only $A_2$ captures two additional answerer-sensitivity effects with similar characteristics. The first, from \citeA{GroenendijkStokhof84_SemanticsOfQuestions}, also emphasizes the role of context: the same wh-question "Where do they sell Italian newspapers in Amsterdam?" is given an \emph{exhaustive} answer (i.e. a complete list of all such locations) in some contexts, and a \emph{mention-some} answer (i.e. a subset of such locations) in others. The second, from 
