...
These case studies center around three empirical observations about the factors that jointly determine answerer behavior in dialogue. First, there is a main effect of the question utterance: given a fixed context, different question utterances elicit different responses. Second, there is a main effect of the context: the \emph{same} question utterance elicits different responses across contexts where different goals are more or less likely. Finally, there is an often subtle interaction between question utterance and context: different question utterances \emph{themselves} indicate different goals \todo[inline]{Not sure we want to go into this, since it's either not about contexts (in which case it's not really an interaction) or we don't have a study to back it up}
