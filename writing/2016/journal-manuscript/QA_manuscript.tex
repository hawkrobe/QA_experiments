...
\abstract{What makes a question useful? What makes an answer helpful? Linguistic theories have highlighted \emph{goal-relevance} as a key to explaining question-answer behavior. In this paper, we propose a computational cognitive model formalizing how answerers reason about possible goals motivating a question and how questioners, in turn, reason about answerers. We test this model against simpler models based on three different pieces of evidence: First, we explore three classic effects in psycholinguistics that show an \emph{answerer}'s level of informativeness varies with the inferred questioner goal. Second, we use a real-time, multi-player communication game to jointly test predictions made by the questioner and answerer components of our model. Third, we conduct a Bayesian data analysis on our model to estimate the extent to which different components are necessary. We find that sophisticated pragmatic reasoning is needed to account for some critical aspects of the data. People can use questions to provide cues to the answerer about their interest, and can select answers that are informative about inferred interests. The best model accurately fits our complex behavioral data, providing a promising direction for understanding the dynamics of human question-answer dialogue.
}
...
\enumsentence{
	...
	A: It's way on the other side of campus, but there's a shop just down the street with a better selection.}
...
In all of the above exchanges, Q strategically chooses a question that manages to send a signal to A about her intentions, while sometimes being unable or unwilling to specify exactly what those intentions are. The direct question or request may be impolite or embarrassing, it may be too long or costly to fully explain, or we may just not know enough about the topic we're interested in to articulate our interests.

A, in turn, reasons beyond the overt question and provides an answer that addresses Q's true interests. Depending on the circumstances, they can adjust their response to be over- or under-informative with respect to the direct question, or to address a different question altogether.  This subtle interplay highlights the prevalence of social reasoning in everyday dialogue and raises two specific questions for formal models of language: What makes a question useful? And what makes an answer helpful? 
