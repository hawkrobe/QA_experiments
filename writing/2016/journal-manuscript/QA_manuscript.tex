...
\item[$\mathcal{A}$:] a set of possible declarative utterances, such as ``It costs \$7.'' or ``Yes, it costs more than \$5.'' We take the literal meanings of these declarative utterances \den{$a$} to be true-conditional: a map from world states to booleans. So the answer ``A fifth costs \$8.'' would be true in the world where $w = \$8$ and false otherwise. 
\end{description}
...
\subsection{Informative Answerer}
...
Formally, $A_0$ hears a question utterance $q \in \mathcal{Q}$ with private knowledge about a world state $w \in \mathcal{W}$. He then uses a soft-max decision rule to choose an utterance $a \in \mathcal{A}$ proportional to the epistemic utility of that utterance to his communication partner: 
$$P_{A_0}(a | q, w) \propto \exp(\alpha U(a;w))$$ 
where $\alpha$ is a soft-max optimality parameter. 

We define the epistemic utility of the utterance using the information-theoretic measure of surprisal: the increase in certainty an imagined interpreter $I$ has about the true state of the world after hearing $a$: $$U(a;w) = \ln P_I(w|a)$$ 

Critically, $A_0$ assumes that $I$ uses Bayesian inference to update their beliefs, conditioning on the literal meaning of the utterance $a$ being true:
$$P_I(w|a) = \delta_{\textrm{\den{a}}(w)} P(w)$$
In other words, the interpreter constrains the prior on worlds to the subset of its support that is consistent with the given utterance.
...
\subsection{Informative and Relevant Answerer}
