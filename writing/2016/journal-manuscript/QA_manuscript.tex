...
Next, having established support for $A_2$ and $Q_2$, we examine the posteriors over their $\beta$ parameters. In particular, because our earlier assumption of uniform saliency $(\beta = 0)$ is nested within our current prior, we use the Savage--Dickey method to test whether there is support for using empirical saliencies. The Savage--Dickey method compares the posterior at the nested model's point value $(\beta = 0)$ with the prior at the same point; the extent to which we \emph{decrease} posterior beliefs relative to the prior is a measure of evidence against the nested model. We found substantial evidence against the nested $\beta=0$ model for both the questioner ($4.10 \times 10^{-14}$) and the answerer ($8.96\times 10^{-21}$), indicating that participants interpret questions using non-uniform saliencies. Finally, the posterior predictives for $A_2$ and $Q_2$ after including empirical saliencies show a substantial proportion of the item-wise variance explained: $R^2 = .92$ and $R^2 = .98$, respectively. Posteriors over alpha are similar to our previous results (see Supplemental).
