...
For a rigorous model comparison among explicit and pragmatic models, we included a binary parameter $\gamma \in \{\textrm{explicit}, \textrm{pragmatic}\}$ determining which model to . The posterior over this parameter can be interpreted as the relative evidence for each model \cite{KruschkeVanPaemel15_OxfordHandbook}.

We infer this parameter, along with the rationality parameter $\alpha$, separately for the questioner and answerer by conditioning on our observed data. We assume $$\gamma \sim Bernoulli(.5)$$ $$\alpha \sim Unif(0,20)$$ and use enumeration over discrete bins to estimate the most likely parameter values (see Figure \ref{fig:BayesianParamPosteriors}). We also show the fit of the posterior predictives of this analysis in Figure \ref{fig:posteriorPredictives}.
