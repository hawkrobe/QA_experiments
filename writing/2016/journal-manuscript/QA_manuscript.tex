...
We place uninformative priors over these parameters $$\gamma \sim \textrm{unif}\{0, 2\}$$ $$\alpha \sim \textrm{unif}(0,20)$$ and perform inference using enumeration over discrete bins. Each model was run with uniform prior probability over worlds, goals, questions, and answers.
...
We next carried out a model comparison using these empirically measured saliency distributions in the question semantics. 
...
Posteriors over alpha parameters are similar to our previous results (see Supplemental).
...
We found maximum a posteriori (MAP) beta values of $\beta = 0.5$ (95\% HPD $=[.4, .55]$) for our answerer model, and $\beta = 0.35$ (95\% HPD interval $= [0.3, 0.45]$) for our questioner model, implying that both questioners and answerers use a mixture between a purely uniform and purely empirical saliency prior. Finally, our model comparison parameter $\gamma$ showed overwhelming evidence for the pragmatic answerer model (BF $= \sim6 \times 10^{72}$) and strong evidence for the pragmatic questioner model (BF $= \sim2\times 10^{7}$).
