...
These results show that our non-pragmatic models cannot account for key qualitative phenomena while our pragmatic models make the correct predictions. Next, we show that our pragmatic models also provide better overall quantitative fits to the behavioral data. Before showing the results of our direct model comparison, we show how we capture the experimental task in our modeling framework. 

The goal projection for goal object $o$ is given by $$g_o(w) = \textrm{loc}(o,w)$$ where $\textrm{loc(o,w)}$ is a simple function looking up the location of object $o$ in world $w$. 
...
\begin{figure*}[t!]
\begin{center}
\includegraphics[scale=.5]{AnswererPredictive.pdf}
\includegraphics[scale=.5]{QuestionerPredictive.pdf}
\end{center}
