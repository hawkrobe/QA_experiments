...
More recent theories have tried to fix these problems by introducing some consideration of the questioner's goals. \citeA{VanRooy03_QuestioningDecisionProblems}, for instance, formalizes these goals as a decision problem faced by the questioner. A useful answer under this decision theoretic account is one that maximizes the expected value of the questioner's decision problem. A useful question is one that induces an appropriately fine-grained partition, optimally distinguishing the worlds relevant to the decision problem without allowing irrelevant distinctions. 


While this framework elegantly accounts for the context-dependence and relevance-maximization of question and answer behavior, it critically assumes that the questioner's decision problem is known \emph{a priori} by the answerer \cite<see>[for a similar proposal in a game theoretic framework where this assumption is relaxed]{StevensBenzReusse___Klabunde14}. \todo[inline]{rxdh: I may be missing something, but the clause ``On the assumption that the answerer knows what the agent's decision problem is\dots'' appears on p. 12 and the issue is never raised again, so I think this is a fair depiction. Still may need  a clearer explanation of ``why van rooij isn't enough / why it needs to be merged into the RSA framework'' though}. The model we propose in the following section is an effort to expand on this core idea in a probabilistic framework, which jointly models the questioner and answerer, makes predictions about the behavior of each, and provides a mechanism for the answerer to \emph{infer} the `decision problem' from observing the context and question utterance.
...
In this paper we extend the RSA framework to address simple question-answer dialogs. This marks the first attempt to generalize RSA beyond single utterance interpretations and the first step toward modeling more complex discourse phenomena.
...
the explicit answerer attempts to be informative with respect to the semantics of the question asked (without inferring the questioner's underlying interests);  
...
%\end{center}
%\vspace{-.25cm}
%\caption{Example `branching' hierarchy used in Exp.~1. The goal space and answer space contained the four leaves. %hidden behind gates (the nodes of the tree). 
...
\label{fig:exp4views}
\end{figure*}
\section{Experiment Testing Questioner Predictions}
