...
var literalQuestioner = cache(function(qud_node, ansRationality, KLRationality) {
...
var expQuestioner = cache(function(qud_node, ansRationality, KLRationality) {
  var qud = (makeQUD(qud_node))
...
var pragmaticAnswerer = cache(function(question, trueWorld, ansR, KLR, qudR, pragR){
  var qudNodePosterior = Enumerate(function(){
    ...
    var q_erp = expQuestioner(qudNode, ansR, KLR);
...
var main = function(){
  ...
  var KLRationality = _.range(1, 8, .5)

  map(function(rAns) {
    map(function(rKL) {
      map(function(qudNode) {
	var erp = literalQuestioner(qudNode, rAns, rKL)
	var label = [qudNode, rAns, rKL]
	console.log(label)
	qa.printERP(erp)
	qa.writeERP(erp, label,
		    "litQuestionerRationalityFitting.csv")
      }, qudNodes)
    }, KLRationality)
  }, ansRationality)
