...
d_a = d %>% 
      ...
      select(domain, type, utterance, response, emp_prob, count)
...
d_a = d_a %>% 
      ...
      do(mutate(., count = ifelse(is.na(count), 0, count))) %>%
      do(mutate(., emp_prob = ifelse(is.na(emp_prob), 0, emp_prob))) 
...
#     group_by(type, modelLevel, rationality, utterance, response, modelProb) %>%
#     summarise(meanEmpProb = mean(emp_prob)) %>%
    group_by(type, modelLevel, rationality) %>%
    filter(rationality > 1) %>%
    summarise(correlation = cor(modelProb, meanEmpProb)) %>%
    mutate(m = max(correlation)) %>%
    ungroup() %>%
    filter(m == correlation) %>%
    mutate(maximizingR = rationality) %>%
    group_by(type, modelLevel, correlation) %>%
    summarize(maximizingR = min(maximizingR)) %>%
    select(type, modelLevel, maximizingR, correlation)
  # add literal back in w/ correlation = NA
  prob_correlation <- rbind(prob_correlation, c('overlapping', 'literal', 1.0, NA))
  prob_correlation <- rbind(prob_correlation, c('equivocal', 'literal', 1.0, NA))
  prob_correlation <- rbind(prob_correlation, c('branching', 'literal', 1.0, NA))
...
ansFits = optimalFit(inner_join(d_a, read.csv("analysis/predictions/answererPredictions.csv", sep = ','))) %>%
  select(modelLevel, type, utterance, response, emp_prob, rationality, modelProb)
```
...
#  ungroup() %>%
  ...
  select(type, domain, utterance, response, emp_prob, modelLevel, modelProb) 
  #distinct(utterance, response, model_level, model_prob)

# Since they won't let us annotate nicely...
  
answer_plots = (ggplot(all_ans, aes(x = modelProb, y = emp_prob))
...
#                               lab = (all_ans %>% group_by(modelLevel) %>% 
#                                     summarise(correlation = paste("r =",
#                                                                   round(cor(emp_prob, modelProb, method = 'pearson'), 2))))$correlation)))
answer_plots               
```
