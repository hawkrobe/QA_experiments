...
```{r}
answerModelOutput=read.csv("../modeling/guessingGame/answererPredictions.csv", 
                            sep = ',') %>% 
  mutate(question = as.character(utterance), 
         answer = as.character(response)) 

for(i in 1:length(answerModelOutput$type)) {
  answerModelOutput[i,]$answer <- mapAnswer(answerModelOutput[i,])
  answerModelOutput[i,]$question <- mapQuestion(answerModelOutput[i,])
}

answerModelOutput = answerModelOutput %>% 
  mutate(utterance = as.factor(question),
         response = as.factor(answer)) %>% 
