...
collapsed_q <- d %>% 
    ...
    mutate(goal = ordered(goalNodes, levels = c("G1", "G2", "G3", "G4"))) %>%
    group_by(type, goal) %>%
    do(getProbsAndCIs(data = ., QorA = 'q', R = 10)) %>%
...
plottableQ = collapsed_q %>% inner_join(pragModelPreds, by = c('goal', 'type', 'response')) %>%
  ...
  do(mutate(., lower_ci = ifelse(src == "model", prob, lower_ci))) %>%
  do(mutate(., upper_ci = ifelse(src == "model", prob, upper_ci)))
...
collapsed_a <- d %>% 
    ...
    do(getProbsAndCIs(data = ., QorA = 'a', R = 10)) %>%
    mutate(empirical = empProb) %>%
