...
Aprobs <- function(data, indices) {
  ...
  pseudocount <- 0#runif(1, max = .25)
  c <- d %>% 
       group_by(domain, type, utterance, response) %>% 
       summarize(count = n()) %>%
       right_join(expand.grid(response = levels(d$response),
                              type = levels(factor(d$type)),
                              domain = levels(factor(d$domain)),
...
getProbsAndCIs <- function(data, QorA, R) {
  ...
  bootObj <- ifelse(QorA == "q", boot(data, Qprobs, R = R), 
                    boot(data,Aprobs, R=R))
  upper_ci <- c()
  lower_ci <- c()
  for(i in 1:4) {
  ...
  cat("bootstraped lower CI to be", lower_ci)
  cat("bootstraped upper CI to be", upper_ci)
...
d_q = d %>% 
      ...
      do(getProbsAndCIs(data = ., QorA = 'q', R = 5)) %>%
      select(goal, type, response, domain, count,
             groupSize, lower_ci, upper_ci, empProb)
```
...
d_a = d %>% 
      ...
      do(getProbsAndCIs(data = ., QorA = 'a', R = 5)) %>%
