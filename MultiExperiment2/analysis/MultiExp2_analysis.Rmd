...
rsq <- function(data, indices) {
  ...
  pseudocount <- runif(1, max = .25)
  c <- d %>% 
       group_by(domain, type, goal, response) %>% 
       summarize(count = n()) %>%
...
getLowerCI <- function(data, R) {

    bootObj <- boot(data = data, statistic = rsq, R = R)
    G1CI = boot.ci(bootObj, index = 1, type = "perc")$percent[4]
    G2CI = boot.ci(bootObj, index = 2, type = "perc")$percent[4]
    G3CI = boot.ci(bootObj, index = 3, type = "perc")$percent[4]
    G4CI = boot.ci(bootObj, index = 4, type = "perc")$percent[4]
    lower_ci = c(G1CI,G2CI,G3CI,G4CI)
    cat("bootstraped CI to be", lower_ci)
#  }
  outputData = data %>% group_by(domain, type, goal, response) %>% 
       summarize(count = n()) %>%
...
preSummarizedQs = d %>% 
      ...
      mutate(group_size = n()) %>%
      select(goal, type, response, group_size)

bootObj = preSummarizedQs %>% group_by(domain, type, goal) %>% 
      do(getLowerCI(data = ., 5))


# d_q_temp = preSummarizedQs %>%
