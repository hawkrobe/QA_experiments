...
getRawPredictives <- function(name, chosenBeta = NA, chosenModelType = NA) {
 inputName <- paste0("../modeling/guessingGame/Bayesian/data/", name, '.csv')
  ...
  output <- raw %>% 
    gather(key, prob, `animal_branching_animals_dalmatian.` : `thing_overlapping_artifact_wooden ruler.`) %>% 
    separate(key, into= c('parameter', 'item1', 'item2', 'value'), sep = '_')
  return(output)
}
View(getRawParams('answerer_modelComparisonParams', chosenBeta = '0', chosenModelType = 'pragmatic'))
View(getRawPredictives('answerer_modelComparisonPredictives', chosenBeta = '0', chosenModelType = 'pragmatic'))
```
...
examinePredictives('answerer_modelComparisonPredictives', useTypicality = F, winningModel = 'pragmatic')
```