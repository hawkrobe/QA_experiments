...
comp.ans = modelComparison("answerer_modelComparisonParams", useTypicality = FALSE) %>% 
  spread(modelType, prob) 
cat('\nBF (beta=0, answerer)', exp(comp.ans$pragmatic - comp.ans$explicit))

comp.quest = modelComparison("questioner_modelComparisonParams", useTypicality = FALSE) %>% 
  spread(modelType, prob) 
cat('\nBF (beta=0, questioner)', exp(comp.quest$pragmatic - comp.quest$explicit))
```
...
```{r}
examineParams('questioner_modelComparisonParams', useTypicality=FALSE, winningModel = 'pragmatic')
```
...
examinePredictives <- function(name, useTypicality, winningModel) {
  ...
  predictive.plot = (ggplot(predictive, aes(x = MAP, y = empProb))
    ...
    + geom_point(aes(colour = domain, shape = type),  size = 2)
  ...
  outputfile <- paste0('../writing/2016/journal-manuscript/figures/',
                       name, '_typ', useTypicality, '_model', winningModel, '.pdf')
...
```{r}
A1 = examinePredictives('answerer_modelComparisonPredictives', useTypicality = F, winningModel = 'explicit')
A2 = examinePredictives('answerer_modelComparisonPredictives', useTypicality = F, winningModel = 'pragmatic')
A3 = examinePredictives('answerer_modelComparisonPredictives', useTypicality = T, winningModel = 'explicit')
A4 = examinePredictives('answerer_modelComparisonPredictives', useTypicality = T, winningModel = 'pragmatic')
...
```{r}
Q1 = examinePredictives('questioner_modelComparisonPredictives', useTypicality = F, winningModel = 'explicit')
Q2 = examinePredictives('questioner_modelComparisonPredictives', useTypicality = F, winningModel = 'pragmatic')
...
```{r}
# Marginalize over alpha
comp.ans = modelComparison("answerer_modelComparisonParams", useTypicality = TRUE) %>% 
  spread(modelType, prob) 
cat('\nBF (beta=0, answerer)', exp(comp.ans$pragmatic - comp.ans$explicit))

comp.quest = modelComparison("questioner_modelComparisonParams", useTypicality = TRUE) %>% 
  spread(modelType, prob) 
cat('\nBF (beta=0, questioner)', exp(comp.quest$pragmatic - comp.quest$explicit))
```
...
```{r}
savageDickey <- function(name) {
  res <- getRawParams(name, chosenModelType = 'pragmatic') %>% 
    group_by(beta) %>% 
    summarize(posterior = reduce(MCMCprob, sumlogprob)) %>%
    mutate(prior = log(1/11)) %>%
    mutate(BF = exp(posterior - prior)) %>%
    filter(beta == 0)
  return(res$BF)
}

savageDickey('answerer_modelComparisonParams')
savageDickey('questioner_modelComparisonParams')
```
...
```{r}
A1 = examinePredictives('answerer_modelComparisonPredictives', useTypicality = T, winningModel = 'explicit')
A2 = examinePredictives('answerer_modelComparisonPredictives', useTypicality = T, winningModel = 'pragmatic')
plot_grid(A1, A2)
```
...
```{r}
Q1 = examinePredictives('questioner_modelComparisonPredictives', useTypicality = T, winningModel = 'explicit')
Q2 = examinePredictives('questioner_modelComparisonPredictives', useTypicality = T, winningModel = 'pragmatic')
plot_grid(Q1, Q2)
```
