  ...
  + geom_text(aes(x,y,label=lab),
              data=data.frame(x = .75, y = .25,
                              ...
                              lab = (all_qs %>% 
                                       ...
                                       summarise(correlation = paste("r =",  round(cor(emp_prob, model_prob, method='kendall'), 2))))$correlation)))
question_plots               
dev.off()
```
...
name = paste("../writing/2015/cogsci/", plot_title, ".jpeg", sep = '')
jpeg(filename = name)
...
```{r}
new_labels = as.factor(sapply(X = prag_ans_fits$utterance, FUN = function(v) {return(paste("utterance:", v))}))
prag_ans_fits$facet_label = ordered(new_labels, 
                              levels = c("utterance: dalmatian", "utterance: dog", 
                                         "utterance: mammal","utterance: animal"))
a_comparison <- prag_ans_fits %>% 
  select(utterance, response, emp_prob, upper_ci, lower_ci, pragmatic_prob, facet_label) %>%
  rename(empirical = emp_prob, model = pragmatic_prob) %>%
  gather(src, prob, empirical, model) 
