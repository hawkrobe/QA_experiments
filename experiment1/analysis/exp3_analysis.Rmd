...
test_dal_a = chisq.test(subset(d_a, qud == "dalmatian")$count) 
test_dal_a
test_poodle_a = chisq.test(subset(d_a, qud == "poodle")$count)
test_poodle_a
test_cat_a = chisq.test(subset(d_a, qud == "siamese cat")$count)
test_cat_a
test_fish_a = chisq.test(subset(d_a, qud == "goldfish")$count)
test_fish_a
```
...
```{r}
jpeg(filename="../writing/2015/cogsci/model_fits_grid.jpeg")
grid.newpage()
grid.draw(rbind(ggplotGrob(answer_plots), ggplotGrob(question_plots), size="last"))
dev.off()
```
